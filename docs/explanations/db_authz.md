...
{ "role": "webuser", "email": "john@doe.com" }

SQL code can access claims through PostgREST [Transaction-Scoped Settings](../references/transactions.md#tx_settings). For instance to get the email claim, call this function:
...
CREATE ROLE webuser NOLOGIN; -- grant this role access to certain tables etc

CREATE ROLE user000 NOLOGIN; GRANT webuser TO user000; -- now user000 can do whatever webuser can

GRANT user000 TO authenticator; -- allow authenticator to switch into user000 role -- (the role itself has nologin)

## Schemas

You must explicitly allow roles to access the exposed schemas in [db-schemas](../references/configuration.md#db-schemas).
...
GRANT SELECT , INSERT , UPDATE(message_body) , DELETE ON chat TO webuser;
...
!!! note
    ...
    This will work because the [alter default privileges` statement has effect on function created *after* it is executed. See `PostgreSQL alter default privileges](https://www.postgresql.org/docs/current/sql-alterdefaultprivileges.html) for more details.
...
A function is executed with the privileges of the user who calls it. This means that the user has to have all permissions to do the operations the function performs. If the function accesses private database objects, your [API roles](../references/auth.md#roles) won't be able to successfully execute the function.

Another option is to define the function with the `SECURITY DEFINER` option. Then only one permission check will take place, the permission to call the function, and the operations in the function will have the authority of the user who owns the function itself.
...
-- create a sample function create or replace function login(email text, pass text, out token text) as $$ begin -- access to a private schema called 'auth' select auth.user_role(email, pass) into _role; -- other operations -- ... end; $$ language plpgsql security definer;

Note the `SECURITY DEFINER` keywords at the end of the function. See [PostgreSQL documentation](https://www.postgresql.org/docs/current/sql-createfunction.html#SQL-CREATEFUNCTION-SECURITY) for more details.

## Views

Views are invoked with the privileges of the view owner, much like functions with the `SECURITY DEFINER` option. When created by a SUPERUSER role, all [row-level security](https://www.postgresql.org/docs/current/ddl-rowsecurity.html) policies will be bypassed.
...
CREATE VIEW sample_view WITH (security_invoker = true) AS SELECT * FROM sample_table;
...
CREATE ROLE api_views_owner NOSUPERUSER NOBYPASSRLS; ALTER VIEW sample_view OWNER TO api_views_owner;
