...
## Introduction
 ...
 This chapter will on occasion refer to examples found in [The SQL Language](../the-sql-language/index.md#tutorial-sql) to change or improve them, so it will be useful to have read that chapter. Some examples from this chapter can also be found in `advanced.sql` in the tutorial directory. This file also contains some sample data to load, which is not repeated here. (Refer to [Introduction](../the-sql-language/introduction.md#tutorial-sql-intro) for how to use the file.)
