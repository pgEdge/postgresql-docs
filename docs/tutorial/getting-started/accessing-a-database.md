...
-  Writing a custom application, using one of the several available language bindings. These possibilities are discussed further in [Client Interfaces](../../client-interfaces/index.md#client-interfaces).
...
mydb=> SELECT version();
                                         version
-------------------------------------------------------------------​-----------------------
 PostgreSQL 19devel on x86_64-pc-linux-gnu, compiled by gcc (Debian 4.9.2-10) 4.9.2, 64-bit
(1 row)
...
mydb=> \q
```
 and `psql` will quit and return you to your command shell. (For more internal commands, type `\?` at the `psql` prompt.) The full capabilities of `psql` are documented in [app-psql](../../reference/postgresql-client-applications/psql.md#app-psql). In this tutorial we will not use these features explicitly, but you can use them yourself when it is helpful.
