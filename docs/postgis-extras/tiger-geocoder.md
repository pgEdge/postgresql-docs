...
This is useful for minimizing index bloat that may confuse the query planner or take up unnecessary space. Use in combination with [Install_Missing_Indexes](#Install_Missing_Indexes) to add just the indexes used by the geocoder.
...
[Install_Missing_Indexes](#Install_Missing_Indexes), [Missing_Indexes_Generate_Script](#Missing_Indexes_Generate_Script)
  <a id="Drop_Nation_Tables_Generate_Script"></a>

# Drop_Nation_Tables_Generate_Script

Generates a script that drops all tables in the specified schema that start with `county_all`, `state_all` or state code followed by `county` or `state`.
...
[Loader_Generate_Nation_Script](#Loader_Generate_Nation_Script)
  <a id="Drop_State_Tables_Generate_Script"></a>
...
[Normalize_Address](#Normalize_Address), [Pprint_Addy](#Pprint_Addy), [ST_AsText](../postgis-reference/geometry-output.md#ST_AsText), [ST_SnapToGrid](../postgis-reference/geometry-editors.md#ST_SnapToGrid), [ST_X](../postgis-reference/geometry-accessors.md#ST_X), [ST_Y](../postgis-reference/geometry-accessors.md#ST_Y)
  <a id="Geocode_Intersection"></a>
...
[Geocode](#Geocode), [Pprint_Addy](#Pprint_Addy), [ST_AsText](../postgis-reference/geometry-output.md#ST_AsText)
  <a id="Get_Geocode_Setting"></a>
...
[Set_Geocode_Setting](#Set_Geocode_Setting)
  <a id="Get_Tract"></a>

# Get_Tract

Returns census tract or field from tract table of where the geometry is located. Default to returning short name of tract.
...
!!! note
    ...
    This function uses the census `tract` which is not loaded by default. If you have already loaded your state table, you can load tract as well as bg, and tabblock using the [Loader_Generate_Census_Script](#Loader_Generate_Census_Script) script.
    ...
    then they will be included by the [Loader_Generate_Script](#Loader_Generate_Script).
...
[Geocode](#Geocode)>
  <a id="Install_Missing_Indexes"></a>

# Install_Missing_Indexes

Finds all tables with key columns used in geocoder joins and filter conditions that are missing used indexes on those columns and will add them.
...
[Loader_Generate_Script](#Loader_Generate_Script), [Missing_Indexes_Generate_Script](#Missing_Indexes_Generate_Script)
  <a id="Loader_Generate_Census_Script"></a>

# Loader_Generate_Census_Script

Generates a shell script for the specified platform for the specified states that will download Tiger census state tract, bg, and tabblocks data tables, stage and load into `tiger_data` schema. Each state script is returned as a separate record.
...
!!! note
    ...
    [Loader_Generate_Script](#Loader_Generate_Script) includes this logic, but if you installed tiger geocoder prior to PostGIS 2.0.0 alpha5, you'll need to run this on the states you have already done to get these additional tables.
...
[Tiger Geocoder Enabling your PostGIS database](../postgis-installation/installing-upgrading-tiger-geocoder-and-loading-data.md#install_tiger_geocoder_extension), [Loader_Generate_Nation_Script](#Loader_Generate_Nation_Script), [Drop_State_Tables_Generate_Script](#Drop_State_Tables_Generate_Script)
  <a id="Loader_Generate_Nation_Script"></a>

# Loader_Generate_Nation_Script

Generates a shell script for the specified platform that loads in the county and state lookup tables.
...
It uses unzip on Linux (7-zip on Windows by default) and wget to do the downloading. It uses [Using the Shapefile Loader](../data-management/loading-spatial-data.md#shp2pgsql_usage) to load in the data.
...
!!! note
    ...
    If you were running `tiger_2010` version and you want to reload as state with newer tiger data, you'll need to for the very first load generate and run drop statements [Drop_Nation_Tables_Generate_Script](#Drop_Nation_Tables_Generate_Script) before you run this script.
...
[Loader_Generate_Script](#Loader_Generate_Script), [Drop_Nation_Tables_Generate_Script](#Drop_Nation_Tables_Generate_Script)
  <a id="Missing_Indexes_Generate_Script"></a>

# Missing_Indexes_Generate_Script

Finds all tables with key columns used in geocoder joins that are missing indexes on those columns and will output the SQL DDL to define the index for those tables.
...
[Loader_Generate_Script](#Loader_Generate_Script), [Install_Missing_Indexes](#Install_Missing_Indexes)
  <a id="Normalize_Address"></a>

# Normalize_Address

Given a textual street address, returns a composite `norm_addy` type that has road suffix, prefix and type standardized, street, streetname etc. broken into separate fields. This function will work with just the lookup data packaged with the tiger_geocoder (no need for tiger census data).
...
12. `address_alphanumeric` Full street number even if it has alpha characters like 17R. Parsing of this is better using [Pagc_Normalize_Address](#Pagc_Normalize_Address) function. Availability: PostGIS 2.4.0.


## Examples


Output select fields. Use [Pprint_Addy](#Pprint_Addy) if you want a pretty textual output.
...
[Geocode](#Geocode), [Pprint_Addy](#Pprint_Addy)
  <a id="Pagc_Normalize_Address"></a>
...
There are slight variations in casing and formatting over the [Normalize_Address](#Normalize_Address).
...
[Normalize_Address](#Normalize_Address), [Geocode](#Geocode)
  <a id="Pprint_Addy"></a>

# Pprint_Addy

Given a `norm_addy` composite type object, returns a pretty print representation of it. Usually used in conjunction with normalize_address.
...
Usually used in conjunction with [Normalize_Address](#Normalize_Address).
...
[Normalize_Address](#Normalize_Address)
  <a id="Reverse_Geocode"></a>

# Reverse_Geocode

Takes a geometry point in a known spatial ref sys and returns a record containing an array of theoretically possible addresses and an array of cross streets. If include_strnum_range = true, includes the street range in the cross streets.
...
Enhanced: 2.4.1 if optional zcta5 dataset is loaded, the reverse_geocode function can resolve to state and zip even if the specific state data is not loaded. Refer to [Loader_Generate_Nation_Script](#Loader_Generate_Nation_Script) for details on loading zcta5 data.
...
For this one we reuse our geocoded example from [Geocode](#Geocode) and we only want the primary address and at most 2 cross streets.
...
[Pprint_Addy](#Pprint_Addy), [Geocode](#Geocode), [Loader_Generate_Nation_Script](#Loader_Generate_Nation_Script)
  <a id="Topology_Load_Tiger"></a>

# Topology_Load_Tiger

Loads a defined region of tiger data into a PostGIS Topology and transforming the tiger data to spatial reference of the topology and snapping to the precision tolerance of the topology.
...
!!! note
    ...
    This function relies on Tiger data as well as the installation of the PostGIS topology module. For more information, refer to [Topology](../topology/index.md#Topology) and [Build configuration](../postgis-installation/compiling-and-install-from-source.md#installation_configuration). If you have not loaded data covering the region of interest, then no topology records will be created. This function will also fail if you have not created a topology using the topology functions.
...
[CreateTopology](../topology/topology-constructors.md#CreateTopology), [CreateTopoGeom](../topology/topogeometry-constructors.md#CreateTopoGeom), [TopologySummary](../topology/topology-and-topogeometry-management.md#TopologySummary), [ValidateTopology](../topology/topology-and-topogeometry-management.md#ValidateTopology)
  <a id="Set_Geocode_Setting"></a>
...
Sets value of specific setting stored in `tiger.geocode_settings` table. Settings allow you to toggle debugging of functions. Later plans will be to control rating with settings. Current list of settings are listed in [Get_Geocode_Setting](#Get_Geocode_Setting).
...
If you run [Geocode](#Geocode) when this function is true, the NOTICE log will output timing and queries.
...
[Get_Geocode_Setting](#Get_Geocode_Setting)
