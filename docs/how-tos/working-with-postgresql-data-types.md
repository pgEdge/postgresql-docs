...
To send raw binary to PostgREST you need a function with a single unnamed parameter of [bytea type](https://www.postgresql.org/docs/current/datatype-binary.html).
...
PostgREST allows you to handle [ranges](https://www.postgresql.org/docs/current/rangetypes.html).
...
!!! note
    ...
    If you don't want to modify casts for built-in types, an option would be to [create a custom type](https://www.postgresql.org/docs/current/sql-createtype.html) for your own `tsrange` and add its own cast.
