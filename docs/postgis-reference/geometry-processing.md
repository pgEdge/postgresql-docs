...
!!! note
    ...
    Buffer output is always a valid polygonal geometry. Buffer can handle invalid inputs, so buffering by distance 0 is sometimes used as a way of repairing invalid polygons. [ST_MakeValid](geometry-validation.md#ST_MakeValid) can also be used for this purpose.
...
| ![image](images/st_buffer01.png)   quad_segs=8 (default)    ```sql  SELECT ST_Buffer(  ST_GeomFromText('POINT(100 90)'),  50, 'quad_segs=8');                  ``` | ![image](images/st_buffer02.png)   quad_segs=2 (lame)    ```sql  SELECT ST_Buffer(  ST_GeomFromText('POINT(100 90)'),  50, 'quad_segs=2');                  ``` |
| ![image](images/st_buffer03.png)   endcap=round join=round (default)    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'endcap=round join=round');                  ``` | ![image](images/st_buffer04.png)   endcap=square    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'endcap=square join=round');                  ``` | ![image](images/st_buffer05.png)   endcap=flat    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'endcap=flat join=round');                  ``` |
| ![image](images/st_buffer07.png)   join=bevel    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'join=bevel');                  ``` | ![image](images/st_buffer06.png)   join=mitre mitre_limit=5.0 (default mitre limit)    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'join=mitre mitre_limit=5.0');                  ``` | ![image](images/st_buffer08.png)   join=mitre mitre_limit=1    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'join=mitre mitre_limit=1.0');                  ``` |
| ![image](images/st_buffer09.png)   side=left    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'side=left');                  ``` | ![image](images/st_buffer10.png)   side=right    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'side=right');                  ``` | ![image](images/st_buffer11.png)   side=left join=mitre    ```sql  SELECT ST_Buffer(  ST_GeomFromText(   'LINESTRING(50 50,150 150,150 50)'  ), 10, 'side=left join=mitre');                  ``` |
| ![image](images/st_buffer12.png)   right-hand-winding, polygon boundary side=left    ```sql  SELECT ST_Buffer( ST_ForceRHR( ST_Boundary(  ST_GeomFromText( 'POLYGON ((50 50, 50 150, 150 150, 150 50, 50 50))'))),  ), 20, 'side=left');                  ``` | ![image](images/st_buffer13.png)   right-hand-winding, polygon boundary side=right    ```sql  SELECT ST_Buffer( ST_ForceRHR( ST_Boundary(  ST_GeomFromText( 'POLYGON ((50 50, 50 150, 150 150, 150 50, 50 50))')) ), 20,'side=right')                  ``` |
...
| ![image](images/st_buildarea02.png)   Input lines | ![image](images/st_buildarea03.png)   Area result |
...
![image](images/st_buildarea01.png)
...
| ![image](images/st_centroid01.png)   Centroid of a `MULTIPOINT` | ![image](images/st_centroid02.png)   Centroid of a `LINESTRING` |
| ![image](images/st_centroid03.png)   Centroid of a `POLYGON` | ![image](images/st_centroid04.png)   Centroid of a `GEOMETRYCOLLECTION` |
...
| ![image](images/st_chaikinsmoothing01.png)   nIterations = 1 | ![image](images/st_chaikinsmoothing02.png)   nIterations = 2 | ![image](images/st_chaikinsmoothing03.png)   nIterations = 3 |
...
| ![image](images/st_chaikinsmoothing04.png)   nIterations = 1 | ![image](images/st_chaikinsmoothing05.png)   nIterations = 2 | ![image](images/st_chaikinsmoothing06.png)   nIterations = 3 |
...
![image](images/st_concavehull01.png)
...
![image](images/st_concavehull02.png)
...
| ![image](images/st_concavehull03.png)   `polygon_hull` | ![image](images/st_concavehull04.png)   `points_hull` |
...
![image](images/st_convexhull01.png)
...
<tr>
<td><p>!<a href="images/st_delaunaytriangles04.png">image</a></p>
...
![image](images/st_generatepoints01.png)
...
![image](images/st_geometricmedian01.png)
...
![image](images/st_linemerge01.png)
...
![image](images/st_linemerge02.png)
...
![image](images/st_linemerge03.png)
...
![image](images/st_maximuminscribedcircle01.png)
...
![image](images/st_maximuminscribedcircle02.png)
...
![image](images/st_largestemptycircle01.png)
...
![image](images/st_largestemptycircle02.png)
...
![image](images/st_minimumboundingcircle01.png)
...
![image](images/st_orientedenvelope01.png)
...
| ![image](images/st_pointonsurface01.png)   Point on surface of a `MULTIPOINT` | ![image](images/st_pointonsurface02.png)   Point on surface of a `LINESTRING` |
| ![image](images/st_pointonsurface03.png)   Point on surface of a `POLYGON` | ![image](images/st_pointonsurface04.png)   Point on surface of a `GEOMETRYCOLLECTION` |
...
![image](images/st_pointonsurface.png)
...
| ![image](images/st_polygonize01.png)   Input lines | ![image](images/st_polygonize02.png)   Polygonized result |
...
| ![image](images/st_sharedpaths01.png)   A multilinestring and a linestring |
| ![image](images/st_sharedpaths02.png)   The shared path of multilinestring and linestring overlaid with original geometries.    ```sql   SELECT ST_AsText(   ST_SharedPaths(     ST_GeomFromText('MULTILINESTRING((26 125,26 200,126 200,126 125,26 125),        (51 150,101 150,76 175,51 150))'),     ST_GeomFromText('LINESTRING(151 100,126 156.25,126 125,90 161, 76 175)')     )   ) As wkt                                  wkt ------------------------------------------------------------- GEOMETRYCOLLECTION(MULTILINESTRING((126 156.25,126 125),  (101 150,90 161),(90 161,76 175)),MULTILINESTRING EMPTY)                ``` |
...
![image](images/st_simplify01.png)
...
![image](images/st_simplify02.png)
...
![image](images/st_simplifypreservetopology01.png)
...
![image](images/st_simplifypreservetopology02.png)
...
![image](images/st_simplifypolygonhull02.png)
...
![image](images/st_simplifypolygonhull03.png)
...
![image](images/st_simplifypolygonhull01.png)
...
![image](images/st_simplifyvw01.png)
...
![image](images/st_simplifyvw02.png)
...
![image](images/st_triangulatepolygon01.png)
...
| ![image](images/st_tesselate03.png)   Original polygon | ![image](images/st_triangulatepolygon02.png)   Triangulated Polygon |
