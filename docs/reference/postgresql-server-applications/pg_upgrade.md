...
```
pg_upgrade-bOLDBINDIR [-B NEWBINDIR]-dOLDCONFIGDIR-DNEWCONFIGDIR [OPTION...]
```
...
:   Move the data directories from the old cluster to the new cluster. Then, replace the catalog files with those generated for the new cluster. This mode can outperform `--link`, `--clone`, `--copy`, and `--copy-file-range`, especially on clusters with many relations.
     ...
     Additionally, once the file transfer step begins, the old cluster will be destructively modified and therefore will no longer be safe to start. See [Reverting to old cluster](#pgupgrade-step-revert) for details.
...
!!! note
    ...
    The steps to upgrade *logical replication clusters* are not covered here; refer to [Upgrade](../../server-administration/logical-replication/upgrade.md#logical-replication-upgrade) for details.


1.

    If you are using a version-specific installation directory, e.g., `/opt/PostgreSQL/19`, you do not need to move the old cluster. The graphical installers all use version-specific installation directories.
...
7.
    ...
    `pg_upgrade` will connect to the old and new servers several times, so you might want to set authentication to `peer` in `pg_hba.conf` or use a `~/.pgpass` file (see [The Password File](../../client-interfaces/libpq-c-library/the-password-file.md#libpq-pgpass)).
8.
...
pg_ctl -D /opt/PostgreSQL/19 stop
```
...
9.
    ...
    If you are upgrading standby servers using methods outlined in section [Upgrade streaming replication and log-shipping standby servers](#pgupgrade-step-replicas), verify that the old standby servers are caught up by running pg_controldata against the old primary and standby clusters. Verify that the “Latest checkpoint location” values match in all clusters. Also, make sure `wal_level` is not set to `minimal` in the `postgresql.conf` file on the new primary cluster.
10.
...
rsync --archive --delete --hard-links --size-only --no-inc-recursive /opt/PostgreSQL/12 \
      /opt/PostgreSQL/19 standby.example.com:/opt/PostgreSQL
```
...
rsync --archive --delete --hard-links --size-only --no-inc-recursive /vol1/pg_tblsp/PG_12_201909212 \
      /vol1/pg_tblsp/PG_19_202307071 standby.example.com:/vol1/pg_tblsp
```
