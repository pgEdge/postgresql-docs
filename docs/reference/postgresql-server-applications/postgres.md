...
<code>-B </code><em>nbuffers</em>
:   Sets the number of shared buffers for use by the server processes. The default value of this parameter is chosen automatically by initdb. Specifying this option is equivalent to setting the [shared_buffers](../../server-administration/server-configuration/resource-consumption.md#guc-shared-buffers) configuration parameter.

<code>-c </code><em>name</em><code>=</code><em>value</em>
:   Sets a named run-time parameter. The configuration parameters supported by PostgreSQL are described in [Server Configuration](../../server-administration/server-configuration/index.md#runtime-config). Most of the other command line options are in fact short forms of such a parameter assignment. `-c` can appear multiple times to set multiple parameters.

<code>-C </code><em>name</em>
:   Prints the value of the named run-time parameter, and exits. (See the `-c` option above for details.) This returns values from `postgresql.conf`, modified by any parameters supplied in this invocation. It does not reflect parameters supplied when the cluster was started.
     ...
     This can be used on a running server for most parameters. However, the server must be shut down for some runtime-computed parameters (e.g., [shared_memory_size](../../server-administration/server-configuration/preset-options.md#guc-shared-memory-size), [shared_memory_size_in_huge_pages](../../server-administration/server-configuration/preset-options.md#guc-shared-memory-size-in-huge-pages), and [wal_segment_size](../../server-administration/server-configuration/preset-options.md#guc-wal-segment-size)).


     This option is meant for other programs that interact with a server instance, such as [app-pg-ctl](pg_ctl.md#app-pg-ctl), to query configuration parameter values. User-facing applications should instead use [`SHOW`](../sql-commands/show.md#sql-show) or the `pg_settings` view.

<code>-d </code><em>debug-level</em>
...
<code>-D </code><em>datadir</em>
:   Specifies the file system location of the database configuration files. See [File Locations](../../server-administration/server-configuration/file-locations.md#runtime-config-file-locations) for details.

`-e`
:   Sets the default date style to “European”, that is `DMY` ordering of input date fields. This also causes the day to be printed before the month in certain date output formats. See [Date/Time Types](../../the-sql-language/data-types/date-time-types.md#datatype-datetime) for more information.

`-F`
:   Disables `fsync` calls for improved performance, at the risk of data corruption in the event of a system crash. Specifying this option is equivalent to disabling the [fsync](../../server-administration/server-configuration/write-ahead-log.md#guc-fsync) configuration parameter. Read the detailed documentation before using this!

<code>-h </code><em>hostname</em>
:   Specifies the IP host name or address on which `postgres` is to listen for TCP/IP connections from client applications. The value can also be a comma-separated list of addresses, or `*` to specify listening on all available interfaces. An empty value specifies not listening on any IP addresses, in which case only Unix-domain sockets can be used to connect to the server. Defaults to listening only on `localhost`. Specifying this option is equivalent to setting the [listen_addresses](../../server-administration/server-configuration/connections-and-authentication.md#guc-listen-addresses) configuration parameter.
...
:   Allows remote clients to connect via TCP/IP (Internet domain) connections. Without this option, only local connections are accepted. This option is equivalent to setting `listen_addresses` to `*` in `postgresql.conf` or via `-h`.
     ...
     This option is deprecated since it does not allow access to the full functionality of [listen_addresses](../../server-administration/server-configuration/connections-and-authentication.md#guc-listen-addresses). It's usually better to set `listen_addresses` directly.

<code>-k </code><em>directory</em>
:   Specifies the directory of the Unix-domain socket on which `postgres` is to listen for connections from client applications. The value can also be a comma-separated list of directories. An empty value specifies not listening on any Unix-domain sockets, in which case only TCP/IP sockets can be used to connect to the server. The default value is normally `/tmp`, but that can be changed at build time. Specifying this option is equivalent to setting the [unix_socket_directories](../../server-administration/server-configuration/connections-and-authentication.md#guc-unix-socket-directories) configuration parameter.

`-l`
:   Enables secure connections using SSL. PostgreSQL must have been compiled with support for SSL for this option to be available. For more information on using SSL, refer to [Secure TCP/IP Connections with SSL](../../server-administration/server-setup-and-operation/secure-tcp-ip-connections-with-ssl.md#ssl-tcp).

<code>-N </code><em>max-connections</em>
:   Sets the maximum number of client connections that this server will accept. The default value of this parameter is chosen automatically by initdb. Specifying this option is equivalent to setting the [max_connections](../../server-administration/server-configuration/connections-and-authentication.md#guc-max-connections) configuration parameter.

<code>-p </code><em>port</em>
...
`-S` *work-mem*
:   Specifies the base amount of memory to be used by sorts and hash tables before resorting to temporary disk files. See the description of the `work_mem` configuration parameter in [Memory](../../server-administration/server-configuration/resource-consumption.md#runtime-config-resource-memory).

`-V`, `--version`
:   Print the postgres version and exit.

<code>--</code><em>name</em><code>=</code><em>value</em>
...
### Options for Single-User Mode
 ...
 The following options only apply to the single-user mode (see [Single-User Mode](#app-postgres-single-user) below).
...
`PGDATESTYLE`
:   Default value of the [DateStyle](../../server-administration/server-configuration/client-connection-defaults.md#guc-datestyle) run-time parameter. (The use of this environment variable is deprecated.)
...
## Diagnostics
 ...
 A failure message mentioning `semget` or `shmget` probably indicates you need to configure your kernel to provide adequate shared memory and semaphores. For more discussion see [Managing Kernel Resources](../../server-administration/server-setup-and-operation/managing-kernel-resources.md#kernel-resources). You might be able to postpone reconfiguring your kernel by decreasing [shared_buffers](../../server-administration/server-configuration/resource-consumption.md#guc-shared-buffers) to reduce the shared memory consumption of PostgreSQL, and/or by reducing [max_connections](../../server-administration/server-configuration/connections-and-authentication.md#guc-max-connections) to reduce the semaphore consumption.
...
## Notes
 ...
 The utility command [app-pg-ctl](pg_ctl.md#app-pg-ctl) can be used to start and shut down the `postgres` server safely and comfortably.
...
## See Also
 ...
 [app-initdb](initdb.md#app-initdb), [app-pg-ctl](pg_ctl.md#app-pg-ctl)
