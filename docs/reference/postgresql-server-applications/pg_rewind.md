...
-  All other files, including new relation files and configuration files, are copied in full.
 ...
 pg_rewind requires that the target server either has the [wal_log_hints](../../server-administration/server-configuration/write-ahead-log.md#guc-wal-log-hints) option enabled in `postgresql.conf` or data checksums enabled when the cluster was initialized with initdb (the default). [full_page_writes](../../server-administration/server-configuration/write-ahead-log.md#guc-full-page-writes) must also be set to `on`, but is enabled by default.


!!! warning "Warning: Failures While Rewinding"
...
## Environment
 ...
 When `--source-server` option is used, pg_rewind also uses the environment variables supported by libpq (see [Environment Variables](../../client-interfaces/libpq-c-library/environment-variables.md#libpq-envars)).
