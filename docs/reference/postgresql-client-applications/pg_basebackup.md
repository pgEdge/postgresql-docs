...
## Description
 ...
 pg_basebackup is used to take a base backup of a running PostgreSQL database cluster. The backup is taken without affecting other clients of the database, and can be used both for point-in-time recovery (see [Continuous Archiving and Point-in-Time Recovery (PITR)](../../server-administration/backup-and-restore/continuous-archiving-and-point-in-time-recovery-pitr.md#continuous-archiving)) and as the starting point for a log-shipping or streaming-replication standby server (see [Log-Shipping Standby Servers](../../server-administration/high-availability-load-balancing-and-replication/log-shipping-standby-servers.md#warm-standby)).


 pg_basebackup can take a full or incremental base backup of the database. When used to take a full backup, it makes an exact copy of the database cluster's files. When used to take an incremental backup, some files that would have been part of a full backup may be replaced with incremental versions of the same files, containing only those blocks that have been modified since the reference backup. An incremental backup cannot be used directly; instead, [app-pgcombinebackup](pg_combinebackup.md#app-pgcombinebackup) must first be used to combine it with the previous backups upon which it depends. See [Making an Incremental Backup](../../server-administration/backup-and-restore/continuous-archiving-and-point-in-time-recovery-pitr.md#backup-incremental-backup) for more information about incremental backups, and [Recovering Using a Continuous Archive Backup](../../server-administration/backup-and-restore/continuous-archiving-and-point-in-time-recovery-pitr.md#backup-pitr-recovery) for steps to recover from a backup.


 In any mode, pg_basebackup makes sure the server is put into and out of backup mode automatically. Backups are always taken of the entire database cluster; it is not possible to back up individual databases or database objects. For selective backups, another tool such as [app-pgdump](pg_dump.md#app-pgdump) must be used.


 The backup is made over a regular PostgreSQL connection that uses the replication protocol. The connection must be made with a user ID that has `REPLICATION` permissions (see [Role Attributes](../../server-administration/database-roles/role-attributes.md#role-attributes)) or is a superuser, and [`pg_hba.conf`](../../server-administration/client-authentication/the-pg_hba-conf-file.md#auth-pg-hba-conf) must permit the replication connection. The server must also be configured with [max_wal_senders](../../server-administration/server-configuration/replication.md#guc-max-wal-senders) set high enough to provide at least one walsender for the backup plus one for WAL streaming (if used).
 ...
 pg_basebackup can make a base backup from not only a primary server but also a standby. To take a backup from a standby, set up the standby so that it can accept replication connections (that is, set `max_wal_senders` and [hot_standby](../../server-administration/server-configuration/replication.md#guc-hot-standby), and configure its `pg_hba.conf` appropriately). You will also need to enable [full_page_writes](../../server-administration/server-configuration/write-ahead-log.md#guc-full-page-writes) on the primary.
...
-  All WAL records required for the backup must contain sufficient full-page writes, which requires you to enable `full_page_writes` on the primary.
 ...
 Whenever pg_basebackup is taking a base backup, the server's `pg_stat_progress_basebackup` view will report the progress of the backup. See [Base Backup Progress Reporting](../../server-administration/monitoring-database-activity/progress-reporting.md#basebackup-progress-reporting) for details.
...
<code>-i </code><em>old_manifest_file</em>, <code>--incremental=</code><em>old_manifest_file</em>
:   Performs an [incremental backup](../../server-administration/backup-and-restore/continuous-archiving-and-point-in-time-recovery-pitr.md#backup-incremental-backup). The backup manifest for the reference backup must be provided, and will be uploaded to the server, which will respond by sending the requested incremental backup.

`-R`, `--write-recovery-conf`
:   Creates a [`standby.signal`](../../server-administration/high-availability-load-balancing-and-replication/log-shipping-standby-servers.md#file-standby-signal)  file and appends connection settings to the `postgresql.auto.conf` file in the target directory (or within the base archive file when using tar format). This eases setting up a standby server using the results of the backup.


     The `postgresql.auto.conf` file will record the connection settings and, if specified, the replication slot that pg_basebackup is using, so that streaming replication and [logical replication slot synchronization](../../server-programming/logical-decoding/logical-decoding-concepts.md#logicaldecoding-replication-slots-synchronization) will use the same settings later on. The dbname will be recorded only if the dbname was specified explicitly in the connection string or [environment variable](../../client-interfaces/libpq-c-library/environment-variables.md#libpq-envars).

<code>-t </code><em>target</em>, <code>--target=</code><em>target</em>
...
<code>-T </code><em>olddir</em><code>=</code><em>newdir</em>, <code>--tablespace-mapping=</code><em>olddir</em><code>=</code><em>newdir</em>
...
<code>-X </code><em>method</em>, <code>--wal-method=</code><em>method</em>
:   Includes the required WAL (write-ahead log) files in the backup. This will include all write-ahead logs generated during the backup. Unless the method `none` is specified, it is possible to start a postmaster in the target directory without the need to consult the WAL archive, thus making the output a completely standalone backup.
    ...
    `f`, `fetch`
    :   The write-ahead log files are collected at the end of the backup. Therefore, it is necessary for the source server's [wal_keep_size](../../server-administration/server-configuration/replication.md#guc-wal-keep-size) parameter to be set high enough that the required log data is not removed before the end of the backup. If the required log data has been recycled before it's time to transfer it, the backup will fail and be unusable.
...
<code>-Z </code><em>level</em>, <code>-Z [{client|server}-]</code><em>method</em><code>[:</code><em>detail</em><code>]</code>, <code>--compress=</code><em>level</em>, <code>--compress=[{client|server}-]</code><em>method</em><code>[:</code><em>detail</em><code>]</code>
...
:   Sets checkpoint mode to fast or spread (see [Making a Base Backup Using the Low Level API](../../server-administration/backup-and-restore/continuous-archiving-and-point-in-time-recovery-pitr.md#backup-lowlevel-base-backup)). The default is spread.
...
<code>-l </code><em>label</em>, <code>--label=</code><em>label</em>
...
<code>-r </code><em>rate</em>, <code>--max-rate=</code><em>rate</em>
...
:   When set to `fsync`, which is the default, `pg_basebackup` will recursively open and synchronize all files in the backup directory. When the plain format is used, the search for files will follow symbolic links for the WAL directory and each configured tablespace.
     ...
     On Linux, `syncfs` may be used instead to ask the operating system to synchronize the whole file system that contains the backup directory. When the plain format is used, `pg_basebackup` will also synchronize the file systems that contain the WAL files and each tablespace. See [recovery_init_sync_method](../../server-administration/server-configuration/error-handling.md#guc-recovery-init-sync-method) for information about the caveats to be aware of when using `syncfs`.
...
<code>--manifest-checksums=</code><em>algorithm</em>
:   Specifies the checksum algorithm that should be applied to each file included in the backup manifest. Currently, the available algorithms are `NONE`, `CRC32C`, `SHA224`, `SHA256`, `SHA384`, and `SHA512`. The default is `CRC32C`.
     ...
     [app-pgverifybackup](pg_verifybackup.md#app-pgverifybackup) can be used to check the integrity of a backup against the backup manifest.
...
`--no-manifest`
:   Disables generation of a backup manifest. If this option is not specified, the server will generate and send a backup manifest which can be verified using [app-pgverifybackup](pg_verifybackup.md#app-pgverifybackup). The manifest is a list of every file present in the backup with the exception of any WAL files that may be included. It also stores the size, last modification time, and an optional checksum for each file.
...
:   Disables verification of checksums, if they are enabled on the server the base backup is taken from.
     ...
     By default, checksums are verified and checksum failures will result in a non-zero exit status. However, the base backup will not be removed in such a case, as if the `--no-clean` option had been used. Checksum verification failures will also be reported in the [`pg_stat_database`](../../server-administration/monitoring-database-activity/the-cumulative-statistics-system.md#monitoring-pg-stat-database-view) view.
...
<code>-s </code><em>interval</em>, <code>--status-interval=</code><em>interval</em>
...
## See Also
  [app-pgdump](pg_dump.md#app-pgdump), [Base Backup Progress Reporting](../../server-administration/monitoring-database-activity/progress-reporting.md#basebackup-progress-reporting)
