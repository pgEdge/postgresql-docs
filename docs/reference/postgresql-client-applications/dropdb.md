...
## Description
 ...
 dropdb is a wrapper around the SQL command [`DROP DATABASE`](../sql-commands/drop-database.md#sql-dropdatabase). There is no effective difference between dropping databases via this utility and via other methods for accessing the server.
...
`-f`, `--force`
:   Attempt to terminate all existing connections to the target database before dropping it. See [sql-dropdatabase](../sql-commands/drop-database.md#sql-dropdatabase) for more information on this option.
...
## Diagnostics
 ...
 In case of difficulty, see [sql-dropdatabase](../sql-commands/drop-database.md#sql-dropdatabase) and [app-psql](psql.md#app-psql) for discussions of potential problems and error messages. The database server must be running at the targeted host. Also, any default connection settings and environment variables used by the libpq front-end library will apply.
...
## See Also
  [app-createdb](createdb.md#app-createdb), [sql-dropdatabase](../sql-commands/drop-database.md#sql-dropdatabase)
