 ...
 *dbname* can also be a [connection string](../../client-interfaces/libpq-c-library/database-connection-control-functions.md#libpq-connstring).
...
<code>-d </code><em>pattern</em>, <code>--database=</code><em>pattern</em>
:   Check databases matching the specified [*pattern*](psql.md#app-psql-patterns), except for any excluded by `--exclude-database`. This option can be specified more than once.

<code>-D </code><em>pattern</em>, <code>--exclude-database=</code><em>pattern</em>
:   Exclude databases matching the given [*pattern*](psql.md#app-psql-patterns). This option can be specified more than once.

<code>-i </code><em>pattern</em>, <code>--index=</code><em>pattern</em>
:   Check indexes matching the specified [*pattern*](psql.md#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.


     This is similar to the `--relation` option, except that it applies only to indexes, not to other relation types.

<code>-I </code><em>pattern</em>, <code>--exclude-index=</code><em>pattern</em>
:   Exclude indexes matching the specified [*pattern*](psql.md#app-psql-patterns). This option can be specified more than once.


     This is similar to the `--exclude-relation` option, except that it applies only to indexes, not other relation types.

<code>-r </code><em>pattern</em>, <code>--relation=</code><em>pattern</em>
:   Check relations matching the specified [*pattern*](psql.md#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.
...
<code>-R </code><em>pattern</em>, <code>--exclude-relation=</code><em>pattern</em>
:   Exclude relations matching the specified [*pattern*](psql.md#app-psql-patterns). This option can be specified more than once.


     As with `--relation`, the [*pattern*](psql.md#app-psql-patterns) may be unqualified, schema-qualified, or database- and schema-qualified.

<code>-s </code><em>pattern</em>, <code>--schema=</code><em>pattern</em>
:   Check tables and indexes in schemas matching the specified [*pattern*](psql.md#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.
...
<code>-S </code><em>pattern</em>, <code>--exclude-schema=</code><em>pattern</em>
:   Exclude tables and indexes in schemas matching the specified [*pattern*](psql.md#app-psql-patterns). This option can be specified more than once.


     As with `--schema`, the pattern may be database-qualified.

<code>-t </code><em>pattern</em>, <code>--table=</code><em>pattern</em>
:   Check tables matching the specified [*pattern*](psql.md#app-psql-patterns), unless they are otherwise excluded. This option can be specified more than once.
...
:   Exclude tables matching the specified [*pattern*](psql.md#app-psql-patterns). This option can be specified more than once.
...
<code>--startblock=</code><em>block</em>
...
<code>--endblock=</code><em>block</em>
...
`--checkunique`
:   For each index with unique constraint checked, verify that no more than one among duplicate entries is visible in the index using [amcheck](../../appendixes/additional-supplied-modules-and-extensions/amcheck-tools-to-verify-table-and-index-consistency.md#amcheck)'s `checkunique` option.

`--heapallindexed`
:   For each index checked, verify the presence of all heap tuples as index tuples in the index using [amcheck](../../appendixes/additional-supplied-modules-and-extensions/amcheck-tools-to-verify-table-and-index-consistency.md#amcheck)'s `heapallindexed` option.

`--parent-check`
:   For each btree index checked, use [amcheck](../../appendixes/additional-supplied-modules-and-extensions/amcheck-tools-to-verify-table-and-index-consistency.md#amcheck)'s `bt_index_parent_check` function, which performs additional checks of parent/child relationships during index checking.
...
`--rootdescend`
:   For each index checked, re-find tuples on the leaf level by performing a new search from the root page for each tuple using [amcheck](../../appendixes/additional-supplied-modules-and-extensions/amcheck-tools-to-verify-table-and-index-consistency.md#amcheck)'s `rootdescend` option.
...
`-U`, <code>--username=</code><em>username</em>
...
<code>-j </code><em>num</em>, <code>--jobs=</code><em>num</em>
...
`--install-missing`, <code>--install-missing=</code><em>schema</em>
:   Install any missing extensions that are required to check the database(s). If not yet installed, each extension's objects will be installed into the given *schema*, or if not specified into schema `pg_catalog`.
     ...
     At present, the only required extension is [amcheck](../../appendixes/additional-supplied-modules-and-extensions/amcheck-tools-to-verify-table-and-index-consistency.md#amcheck).
...
## Environment
 ...
 `pg_amcheck`, like most other PostgreSQL utilities, also uses the environment variables supported by libpq (see [Environment Variables](../../client-interfaces/libpq-c-library/environment-variables.md#libpq-envars)).
...
## See Also
  [amcheck](../../appendixes/additional-supplied-modules-and-extensions/amcheck-tools-to-verify-table-and-index-consistency.md#amcheck)
