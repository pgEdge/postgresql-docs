...
## Description
 ...
 Whenever the command <code>NOTIFY </code><em>channel</em> is invoked, either by this session or another one connected to the same database, all the sessions currently listening on that notification channel are notified, and each will in turn notify its connected client application.
...
## Notes
 ...
 [sql-notify](notify.md#sql-notify) contains a more extensive discussion of the use of `LISTEN` and `NOTIFY`.
...
## See Also
  [sql-notify](notify.md#sql-notify), [sql-unlisten](unlisten.md#sql-unlisten), [max_notify_queue_pages](../../server-administration/server-configuration/resource-consumption.md#guc-max-notify-queue-pages)
