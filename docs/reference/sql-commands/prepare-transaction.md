...
## Description
 ...
 Once prepared, a transaction can later be committed or rolled back with [`COMMIT PREPARED`](commit-prepared.md#sql-commit-prepared) or [`ROLLBACK PREPARED`](rollback-prepared.md#sql-rollback-prepared), respectively. Those commands can be issued from any session, not only the one that executed the original transaction.
...
## Notes
 ...
 This command must be used inside a transaction block. Use [`BEGIN`](begin.md#sql-begin) to start one.
...
## See Also
  [sql-commit-prepared](commit-prepared.md#sql-commit-prepared), [sql-rollback-prepared](rollback-prepared.md#sql-rollback-prepared)
