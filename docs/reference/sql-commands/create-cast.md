...
```
 ...
 By default, a cast can be invoked only by an explicit cast request, that is an explicit <code>CAST(</code><em>x</em><code> AS
   </code><em>typename</em><code>)</code> or *x*`::`*typename* construct.
...
!!! note
    ...
    Sometimes it is necessary for usability or standards-compliance reasons to provide multiple implicit casts among a set of types, resulting in ambiguity that cannot be avoided as above. The parser has a fallback heuristic based on *type categories* and *preferred types* that can help to provide desired behavior in such cases. See [sql-createtype](create-type.md#sql-createtype) for more information.
...
<em>function_name</em><code>[(</code><em>argument_type</em><code> [, ...])]</code>
