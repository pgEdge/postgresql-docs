...
:   Specific types of privileges, as defined in [Privileges](../../the-sql-language/data-definition/privileges.md#ddl-priv).
...
### GRANT on Roles
 ...
 The `INHERIT` option controls the inheritance status of the new membership; see [Role Membership](../../server-administration/database-roles/role-membership.md#role-membership) for details on inheritance. If it is set to `TRUE`, it causes the new member to inherit from the granted role. If set to `FALSE`, the new member does not inherit. If unspecified when creating a new role membership, this defaults to the inheritance attribute of the new member.


 The `SET` option, if it is set to `TRUE`, allows the member to change to the granted role using the [`SET ROLE`](set-role.md#sql-set-role) command. If a role is an indirect member of another role, it can use `SET ROLE` to change to that role only if there is a chain of grants each of which has `SET TRUE`. This option defaults to `TRUE`.
...
## See Also
  [sql-revoke](revoke.md#sql-revoke), [sql-alterdefaultprivileges](alter-default-privileges.md#sql-alterdefaultprivileges)
