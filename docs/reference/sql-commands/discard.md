...
`SEQUENCES`
:   Discards all cached sequence-related state, including `currval()`/`lastval()` information and any preallocated sequence values that have not yet been returned by `nextval()`. (See [sql-createsequence](create-sequence.md#sql-createsequence) for a description of preallocated sequence values.)
