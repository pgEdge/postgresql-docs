...
*argtype*
:   The data type(s) of the procedure's arguments (optionally schema-qualified), if any. See [sql-dropprocedure](drop-procedure.md#sql-dropprocedure) for the details of how the procedure is looked up using the argument data type(s).
...
`[ EXTERNAL ] SECURITY INVOKER`, `[ EXTERNAL ] SECURITY DEFINER`
:   Change whether the procedure is a security definer or not. The key word `EXTERNAL` is ignored for SQL conformance. See [sql-createprocedure](create-procedure.md#sql-createprocedure) for more information about this capability.
...
## See Also
  [sql-createprocedure](create-procedure.md#sql-createprocedure), [sql-dropprocedure](drop-procedure.md#sql-dropprocedure), [sql-alterfunction](alter-function.md#sql-alterfunction), [sql-alterroutine](alter-routine.md#sql-alterroutine)
