...
*source_query*
:   A query (`SELECT` statement or `VALUES` statement) that supplies the rows to be merged into the target table. Refer to the [sql-select](select.md#sql-select) statement or [sql-values](values.md#sql-values) statement for a description of the syntax.
...
*merge_delete*
:   Specifies a `DELETE` action that deletes the current row of the target table. Do not include the table name or any other clauses, as you would normally do with a [sql-delete](delete.md#sql-delete) command.
...
3.  Perform any `AFTER STATEMENT` triggers for actions specified, whether or not they actually occur. This is similar to the behavior of an `UPDATE` statement that modifies no rows.
 ...
 When `MERGE` is run concurrently with other commands that modify the target table, the usual transaction isolation rules apply; see [Transaction Isolation](../../the-sql-language/concurrency-control/transaction-isolation.md#transaction-iso) for an explanation on the behavior at each isolation level. You may also wish to consider using `INSERT ... ON CONFLICT` as an alternative statement which offers the ability to run an `UPDATE` or return the existing row (with `DO SELECT`) if a concurrent `INSERT` occurs. There are a variety of differences and restrictions between the two statement types and they are not interchangeable.
