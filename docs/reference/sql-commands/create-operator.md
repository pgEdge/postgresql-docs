...
-  The symbol `=>` is reserved by the SQL grammar, so it cannot be used as an operator name.
 ...
 The other clauses specify optional operator optimization clauses. Their meaning is detailed in [Operator Optimization Information](../../server-programming/extending-sql/operator-optimization-information.md#xoper-optimization).
...
## Notes
 ...
 Refer to [User-Defined Operators](../../server-programming/extending-sql/user-defined-operators.md#xoper) for further information.


 It is not possible to specify an operator's lexical precedence in `CREATE OPERATOR`, because the parser's precedence behavior is hard-wired. See [Operator Precedence](../../the-sql-language/sql-syntax/lexical-structure.md#sql-precedence) for precedence details.
 ...
 Use [`DROP OPERATOR`](drop-operator.md#sql-dropoperator) to delete user-defined operators from a database. Use [`ALTER OPERATOR`](alter-operator.md#sql-alteroperator) to modify operators in a database.
...
## See Also
  [sql-alteroperator](alter-operator.md#sql-alteroperator), [sql-createopclass](create-operator-class.md#sql-createopclass), [sql-dropoperator](drop-operator.md#sql-dropoperator)
