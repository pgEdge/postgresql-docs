...
*query*
:   A [`SELECT`](select.md#sql-select), [`VALUES`](values.md#sql-values), [`INSERT`](insert.md#sql-insert), [`UPDATE`](update.md#sql-update), or [`DELETE`](delete.md#sql-delete) command whose results are to be copied. Note that parentheses are required around the query.
...
## Notes
 ...
 `COPY TO` can be used only with plain tables, not views, and does not copy rows from child tables or child partitions. For example, <code>COPY </code><em>table</em><code> TO</code> copies the same rows as <code>SELECT * FROM ONLY </code><em>table</em>. The syntax <code>COPY (SELECT * FROM </code><em>table</em><code>) TO ...</code> can be used to dump all of the rows in an inheritance hierarchy, partitioned table, or view.
