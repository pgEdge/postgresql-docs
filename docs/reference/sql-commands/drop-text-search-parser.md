...
`CASCADE`
:   Automatically drop objects that depend on the text search parser, and in turn all objects that depend on those objects (see [Dependency Tracking](../../the-sql-language/data-definition/dependency-tracking.md#ddl-depend)).
...
## See Also
  [sql-altertsparser](alter-text-search-parser.md#sql-altertsparser), [sql-createtsparser](create-text-search-parser.md#sql-createtsparser)
