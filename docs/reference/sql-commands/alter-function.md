...
`CALLED ON NULL INPUT`, `RETURNS NULL ON NULL INPUT`, `STRICT`
:   `CALLED ON NULL INPUT` changes the function so that it will be invoked when some or all of its arguments are null. `RETURNS NULL ON NULL INPUT` or `STRICT` changes the function so that it is not invoked if any of its arguments are null; instead, a null result is assumed automatically. See [sql-createfunction](create-function.md#sql-createfunction) for more information.

`IMMUTABLE`, `STABLE`, `VOLATILE`
:   Change the volatility of the function to the specified setting. See [sql-createfunction](create-function.md#sql-createfunction) for details.

`[ EXTERNAL ] SECURITY INVOKER`, `[ EXTERNAL ] SECURITY DEFINER`
:   Change whether the function is a security definer or not. The key word `EXTERNAL` is ignored for SQL conformance. See [sql-createfunction](create-function.md#sql-createfunction) for more information about this capability.

`PARALLEL`
:   Change whether the function is deemed safe for parallelism. See [sql-createfunction](create-function.md#sql-createfunction) for details.

`LEAKPROOF`
:   Change whether the function is considered leakproof or not. See [sql-createfunction](create-function.md#sql-createfunction) for more information about this capability.

`COST` *execution_cost*
:   Change the estimated execution cost of the function. See [sql-createfunction](create-function.md#sql-createfunction) for more information.

`ROWS` *result_rows*
:   Change the estimated number of rows returned by a set-returning function. See [sql-createfunction](create-function.md#sql-createfunction) for more information.

`SUPPORT` *support_function*
:   Set or change the planner support function to use for this function. See [Function Optimization Information](../../server-programming/extending-sql/function-optimization-information.md#xfunc-optimization) for details. You must be superuser to use this option.
...
## See Also
  [sql-createfunction](create-function.md#sql-createfunction), [sql-dropfunction](drop-function.md#sql-dropfunction), [sql-alterprocedure](alter-procedure.md#sql-alterprocedure), [sql-alterroutine](alter-routine.md#sql-alterroutine)
