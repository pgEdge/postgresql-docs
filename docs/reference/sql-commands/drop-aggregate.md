...
`CASCADE`
:   Automatically drop objects that depend on the aggregate function (such as views using it), and in turn all objects that depend on those objects (see [Dependency Tracking](../../the-sql-language/data-definition/dependency-tracking.md#ddl-depend)).
...
## Notes
 ...
 Alternative syntaxes for referencing ordered-set aggregates are described under [sql-alteraggregate](alter-aggregate.md#sql-alteraggregate).
...
## See Also
  [sql-alteraggregate](alter-aggregate.md#sql-alteraggregate), [sql-createaggregate](create-aggregate.md#sql-createaggregate)
