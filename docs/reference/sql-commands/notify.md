...
## Description
 ...
 The `NOTIFY` command sends a notification event together with an optional “payload” string to each client application that has previously executed <code>LISTEN </code><em>channel</em> for the specified channel name in the current database. Notifications are visible to all users.
...
## Notes
 ...
 The function `pg_notification_queue_usage` returns the fraction of the queue that is currently occupied by pending notifications. See [System Information Functions and Operators](../../the-sql-language/functions-and-operators/system-information-functions-and-operators.md#functions-info) for more information.
...
## See Also
  [sql-listen](listen.md#sql-listen), [sql-unlisten](unlisten.md#sql-unlisten), [max_notify_queue_pages](../../server-administration/server-configuration/resource-consumption.md#guc-max-notify-queue-pages)
