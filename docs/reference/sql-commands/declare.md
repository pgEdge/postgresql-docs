...
## Description
 ...
 `DECLARE` allows a user to create cursors, which can be used to retrieve a small number of rows at a time out of a larger query. After the cursor is created, rows are fetched from it using [`FETCH`](fetch.md#sql-fetch).


!!! note

    This page describes usage of cursors at the SQL command level. If you are trying to use cursors inside a PL/pgSQL function, the rules are different — see [Cursors](../../server-programming/pl-pgsql-sql-procedural-language/cursors.md#plpgsql-cursors).
...
*query*
:   A [`SELECT`](select.md#sql-select) or [`VALUES`](values.md#sql-values) command which will provide the rows to be returned by the cursor.
...
!!! note
 ...
 Unless `WITH HOLD` is specified, the cursor created by this command can only be used within the current transaction. Thus, `DECLARE` without `WITH HOLD` is useless outside a transaction block: the cursor would survive only to the completion of the statement. Therefore PostgreSQL reports an error if such a command is used outside a transaction block. Use [`BEGIN`](begin.md#sql-begin) and [`COMMIT`](commit.md#sql-commit) (or [`ROLLBACK`](rollback.md#sql-rollback)) to define a transaction block.
...
!!! caution
    ...
    Scrollable cursors may give unexpected results if they invoke any volatile functions (see [Function Volatility Categories](../../server-programming/extending-sql/function-volatility-categories.md#xfunc-volatility)). When a previously fetched row is re-fetched, the functions might be re-executed, perhaps leading to results different from the first time. It's best to specify `NO SCROLL` for a query involving volatile functions. If that is not practical, one workaround is to declare the cursor `SCROLL WITH HOLD` and commit the transaction before reading any rows from it. This will force the entire output of the cursor to be materialized in temporary storage, so that volatile functions are executed exactly once for each row.


 If the cursor's query includes `FOR UPDATE` or `FOR SHARE`, then returned rows are locked at the time they are first fetched, in the same way as for a regular [`SELECT`](select.md#sql-select) command with these options. In addition, the returned rows will be the most up-to-date versions.
...
```
 See [sql-fetch](fetch.md#sql-fetch) for more examples of cursor usage.
...
## See Also
  [sql-close](close.md#sql-close), [sql-fetch](fetch.md#sql-fetch), [sql-move](move.md#sql-move)
