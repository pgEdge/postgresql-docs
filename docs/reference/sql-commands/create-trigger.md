...
<tbody>
<tr>
<td rowspan="1"><code>BEFORE</code></td>
<td><code>INSERT</code>/<code>UPDATE</code>/<code>DELETE</code></td>
<td>Tables and foreign tables</td>
<td>Tables, views, and foreign tables</td>
</tr>
<tr>
...
<tr>
<td rowspan="1"><code>AFTER</code></td>
<td><code>INSERT</code>/<code>UPDATE</code>/<code>DELETE</code></td>
<td>Tables and foreign tables</td>
<td>Tables, views, and foreign tables</td>
</tr>
...
<tr>
<td rowspan="1"><code>INSTEAD OF</code></td>
<td><code>INSERT</code>/<code>UPDATE</code>/<code>DELETE</code></td>
<td>Views</td>
<td>—</td>
</tr>
...
<td>—</td>
 ...
 When the `CONSTRAINT` option is specified, this command creates a *constraint trigger*. This is the same as a regular trigger except that the timing of the trigger firing can be adjusted using [`SET CONSTRAINTS`](set-constraints.md#sql-set-constraints). Constraint triggers must be `AFTER ROW` triggers on plain tables (not foreign tables). They can be fired either at the end of the statement causing the triggering event, or at the end of the containing transaction; in the latter case they are said to be *deferred*. A pending deferred-trigger firing can also be forced to happen immediately by using `SET CONSTRAINTS`. Constraint triggers are expected to raise an exception when the constraints they implement are violated.
 ...
 Refer to [Triggers](../../server-programming/triggers/index.md#triggers) for more information about triggers.
...
:   The default timing of the trigger. See the [sql-createtable](create-table.md#sql-createtable) documentation for details of these constraint options. This can only be specified for constraint triggers.
...
*condition*
:   A Boolean expression that determines whether the trigger function will actually be executed. If `WHEN` is specified, the function will only be called if the *condition* returns `true`. In `FOR EACH ROW` triggers, the `WHEN` condition can refer to columns of the old and/or new row values by writing <code>OLD.</code><em>column_name</em> or <code>NEW.</code><em>column_name</em> respectively. Of course, `INSERT` triggers cannot refer to `OLD` and `DELETE` triggers cannot refer to `NEW`.
...
## Notes
 ...
 Use [`DROP TRIGGER`](drop-trigger.md#sql-droptrigger) to remove a trigger.
...
## See Also
  [sql-altertrigger](alter-trigger.md#sql-altertrigger), [sql-droptrigger](drop-trigger.md#sql-droptrigger), [sql-createfunction](create-function.md#sql-createfunction), [sql-set-constraints](set-constraints.md#sql-set-constraints)
