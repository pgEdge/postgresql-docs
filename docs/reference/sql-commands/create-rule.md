...
!!! note
    ...
    A view that is simple enough to be automatically updatable (see [sql-createview](create-view.md#sql-createview)) does not require a user-created rule in order to be updatable. While you can create an explicit rule anyway, the automatic update transformation will generally outperform an explicit rule.


     Another alternative worth considering is to use `INSTEAD OF` triggers (see [sql-createtrigger](create-trigger.md#sql-createtrigger)) in place of rules.
...
## See Also
  [sql-alterrule](alter-rule.md#sql-alterrule), [sql-droprule](drop-rule.md#sql-droprule)
