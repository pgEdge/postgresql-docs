...
See [The config.py File](config_py.md#config_py) for more information on configuration settings.
...
4. Change the ownership of the configuration database to the user that the web server processes will run as, for example, assuming that the web server runs as user www-data in group www-data, and that the SQLite path is `/var/lib/pgadmin4/pgadmin4.db`:
