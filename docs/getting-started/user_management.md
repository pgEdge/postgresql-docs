...
To discard a user, and revoke access to pgAdmin, click the trash icon to the left of the row and confirm deletion in the *Delete user?* dialog. If the user has created some shared servers, then the [Change Ownership](change_ownership.md#change_ownership) dialog will appear to change the ownership of a shared server.
...
To delete a role, click the trash icon to the left of the row and confirm deletion in the *Delete role?* dialog. If the role is associated with any users or resources, you may need to reassign those associations before deletion.

Roles allow administrators to group privileges and assign them to users more efficiently. This helps in managing permissions and access control within the pgAdmin client.
...
To delete the user, invoke `setup.py` with `delete-user` command line option, followed by username and auth_source. For Internal users, email adress will be used instead of username. the option `--yes` is available to skip the confirmation prompt before deleting the user.
...
To bulk import users from a JSON file, invoke `setup.py` with `load-users` command line option, followed by the path to the JSON file.
