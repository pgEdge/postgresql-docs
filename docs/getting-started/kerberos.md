...
If the *pgAdmin* server is under the Apache Server, then you need to add the following parameters in *Directory* directive of [Apache HTTPD Configuration](deployment/server_deployment.md#server_deployment):
...
- To connect the PostgreSQL server with Kerberos authentication, GSSAPI support
    ...
    [configuration](https://www.postgresql.org/docs/current/gssapi-auth.html)
    has to be in place.
