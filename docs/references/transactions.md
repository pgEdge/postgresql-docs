...
!!! note
    ...
    PostgREST provided headers such as `Content-Type`, `Location`, etc. can be overridden this way. Note that irrespective of overridden `Content-Type` response header, the content will still be converted to JSON, unless you use [Media Type Handlers](../api/references/api/media_type_handlers.md#custom_media).
