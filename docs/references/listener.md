...
NOTIFY pgrst, 'reload schema'; -- reload schema cache NOTIFY pgrst, 'reload config'; -- reload config NOTIFY pgrst;                  -- reload both
...
-- check if the instance is a replica postgres=# select pg_is_in_recovery();

### pg_is_in_recovery

t (1 row)

postgres=# LISTEN pgrst; ERROR:  cannot execute LISTEN during recovery
