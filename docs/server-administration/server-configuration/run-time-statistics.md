...
:   Enables timing of cost-based vacuum delay (see [Cost-based Vacuum Delay](vacuuming.md#runtime-config-resource-vacuum-cost)). This parameter is off by default, as it will repeatedly query the operating system for the current time, which may cause significant overhead on some platforms. You can use the [pgtesttiming](../../reference/postgresql-server-applications/pg_test_timing.md#pgtesttiming) tool to measure the overhead of timing on your system. Cost-based vacuum delay timing information is displayed in [`pg_stat_progress_vacuum`](../monitoring-database-activity/progress-reporting.md#vacuum-progress-reporting), [`pg_stat_progress_analyze`](../monitoring-database-activity/progress-reporting.md#analyze-progress-reporting), in the output of [sql-vacuum](../../reference/sql-commands/vacuum.md#sql-vacuum) and [sql-analyze](../../reference/sql-commands/analyze.md#sql-analyze) when the `VERBOSE` option is used, and by autovacuum for auto-vacuums and auto-analyzes when [log_autovacuum_min_duration](error-reporting-and-logging.md#guc-log-autovacuum-min-duration) is set. Only superusers and users with the appropriate `SET` privilege can change this setting.
<a id="guc-track-io-timing"></a>

`track_io_timing` (`boolean`)
:   Enables timing of database I/O waits. This parameter is off by default, as it will repeatedly query the operating system for the current time, which may cause significant overhead on some platforms. You can use the [pgtesttiming](../../reference/postgresql-server-applications/pg_test_timing.md#pgtesttiming) tool to measure the overhead of timing on your system. I/O timing information is displayed in [`pg_stat_database`](../monitoring-database-activity/the-cumulative-statistics-system.md#monitoring-pg-stat-database-view), [`pg_stat_io`](../monitoring-database-activity/the-cumulative-statistics-system.md#monitoring-pg-stat-io-view) (if `object` is not `wal`), in the output of the [`pg_stat_get_backend_io()`](../monitoring-database-activity/the-cumulative-statistics-system.md#pg-stat-get-backend-io) function (if `object` is not `wal`), in the output of [sql-explain](../../reference/sql-commands/explain.md#sql-explain) when the `BUFFERS` option is used, in the output of [sql-vacuum](../../reference/sql-commands/vacuum.md#sql-vacuum) when the `VERBOSE` option is used, by autovacuum for auto-vacuums and auto-analyzes, when [log_autovacuum_min_duration](error-reporting-and-logging.md#guc-log-autovacuum-min-duration) is set and by [pg_stat_statements](../../appendixes/additional-supplied-modules-and-extensions/pg_stat_statements-track-statistics-of-sql-planning-and-execution.md#pgstatstatements). Only superusers and users with the appropriate `SET` privilege can change this setting.
<a id="guc-track-wal-io-timing"></a>

`track_wal_io_timing` (`boolean`)
:   Enables timing of WAL I/O waits. This parameter is off by default, as it will repeatedly query the operating system for the current time, which may cause significant overhead on some platforms. You can use the pg_test_timing tool to measure the overhead of timing on your system. I/O timing information is displayed in [`pg_stat_io`](../monitoring-database-activity/the-cumulative-statistics-system.md#monitoring-pg-stat-io-view) for the `object` `wal` and in the output of the [`pg_stat_get_backend_io()`](../monitoring-database-activity/the-cumulative-statistics-system.md#pg-stat-get-backend-io) function for the `object` `wal`. Only superusers and users with the appropriate `SET` privilege can change this setting.
...
:   Enables in-core computation of a query identifier. Query identifiers can be displayed in the [`pg_stat_activity`](../monitoring-database-activity/the-cumulative-statistics-system.md#monitoring-pg-stat-activity-view) view, using `EXPLAIN`, or emitted in the log if configured via the [log_line_prefix](error-reporting-and-logging.md#guc-log-line-prefix) parameter. The [pg_stat_statements](../../appendixes/additional-supplied-modules-and-extensions/pg_stat_statements-track-statistics-of-sql-planning-and-execution.md#pgstatstatements) extension also requires a query identifier to be computed. Note that an external module can alternatively be used if the in-core query identifier computation method is not acceptable. In this case, in-core computation must be always disabled. Valid values are `off` (always disabled), `on` (always enabled), `auto`, which lets modules such as [pg_stat_statements](../../appendixes/additional-supplied-modules-and-extensions/pg_stat_statements-track-statistics-of-sql-planning-and-execution.md#pgstatstatements) automatically enable it, and `regress` which has the same effect as `auto`, except that the query identifier is not shown in the `EXPLAIN` output in order to facilitate automated regression testing. The default is `auto`.
