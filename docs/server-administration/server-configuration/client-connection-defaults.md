...
:   A path to search for extensions, specifically extension control files (<em>name</em><code>.control</code>). The remaining extension script and secondary control files are then loaded from the same directory where the primary control file was found. See [Extension Files](../../server-programming/extending-sql/packaging-related-objects-into-an-extension.md#extend-extensions-files) for details.
    ...
    ```
     ...
     Note that if you set this parameter to be able to load extensions from nonstandard locations, you will most likely also need to set [dynamic_library_path](#guc-dynamic-library-path) to a correspondent location, for example,
...
:   Soft upper limit of the size of the set returned by GIN index scans. For more information see [GIN Tips and Tricks](../../internals/built-in-index-access-methods/gin-indexes.md#gin-tips).
