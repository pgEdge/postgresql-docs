...
## Logical Replication Failover
 ...
 Because the slot synchronization logic copies asynchronously, it is necessary to confirm that replication slots have been synced to the standby server before the failover happens. To ensure a successful failover, the standby server must be ahead of the subscriber. This can be achieved by configuring [`synchronized_standby_slots`](../server-configuration/replication.md#guc-synchronized-standby-slots).
