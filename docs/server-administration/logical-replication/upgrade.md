...
### Prepare for Publisher Upgrades
 ...
 Before you start upgrading the publisher cluster, ensure that the subscription is temporarily disabled, by executing [`ALTER SUBSCRIPTION ... DISABLE`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription). Re-enable the subscription after the upgrade.
...
-  The new cluster must have [`wal_level`](../server-configuration/write-ahead-log.md#guc-wal-level) as `replica` or `logical`.
...
-  All the subscription tables in the old subscriber should be in state `i` (initialize) or `r` (ready). This can be verified by checking [pg_subscription_rel](../../internals/system-catalogs/pg_subscription_rel.md#catalog-pg-subscription-rel).`srsubstate`.
...
!!! note
    ...
    The logical replication restrictions apply to logical replication cluster upgrades also. See [Restrictions](restrictions.md#logical-replication-restrictions) for details.
...
!!! warning
    ...
    Upgrading logical replication cluster requires multiple steps to be performed on various nodes. Because not all operations are transactional, the user is advised to take backups as described in [Making a Base Backup](../backup-and-restore/continuous-archiving-and-point-in-time-recovery-pitr.md#backup-base-backup).
...
10.  On `node2`, create any tables that were created in the upgraded publisher `node1` server between [two-node-cluster-disable-subscriptions-node2](#two-node-cluster-disable-subscriptions-node2) and now, e.g.:
...
11.  Enable all the subscriptions on `node2` that are subscribing the changes from `node1` by using [`ALTER SUBSCRIPTION ... ENABLE`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-enable), e.g.:
...
12.  Refresh the `node2` subscription's publications using [`ALTER SUBSCRIPTION ... REFRESH PUBLICATION`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-refresh-publication), e.g.:
...
11.  On `node2`, create any tables that were created in the upgraded publisher `node1` server between [cascaded-cluster-disable-sub-node1-node2](#cascaded-cluster-disable-sub-node1-node2) and now, e.g.:
...
12.  Enable all the subscriptions on `node2` that are subscribing the changes from `node1` by using [`ALTER SUBSCRIPTION ... ENABLE`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-enable), e.g.:
...
13.  Refresh the `node2` subscription's publications using [`ALTER SUBSCRIPTION ... REFRESH PUBLICATION`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-refresh-publication), e.g.:
...
18.  On `node3`, create any tables that were created in the upgraded `node2` between [cascaded-cluster-disable-sub-node2-node3](#cascaded-cluster-disable-sub-node2-node3) and now, e.g.:
...
19.  Enable all the subscriptions on `node3` that are subscribing the changes from `node2` by using [`ALTER SUBSCRIPTION ... ENABLE`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-enable), e.g.:
...
20.  Refresh the `node3` subscription's publications using [`ALTER SUBSCRIPTION ... REFRESH PUBLICATION`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-refresh-publication), e.g.:
...
6.  Enable all the subscriptions on `node2` that are subscribing the changes from `node1` by using [`ALTER SUBSCRIPTION ... ENABLE`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-enable), e.g.:
...
7.  On `node1`, create any tables that were created in `node2` between [circular-cluster-disable-sub-node2](#circular-cluster-disable-sub-node2) and now, e.g.:
...
8.  Refresh the `node1` subscription's publications to copy initial table data from `node2` using [`ALTER SUBSCRIPTION ... REFRESH PUBLICATION`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-refresh-publication), e.g.:
...
14.  Enable all the subscriptions on `node1` that are subscribing the changes from `node2` by using [`ALTER SUBSCRIPTION ... ENABLE`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-enable), e.g.:
...
15.  On `node2`, create any tables that were created in the upgraded `node1` between [circular-cluster-disable-sub-node1](#circular-cluster-disable-sub-node1) and now, e.g.:
...
16.  Refresh the `node2` subscription's publications to copy initial table data from `node1` using [`ALTER SUBSCRIPTION ... REFRESH PUBLICATION`](../../reference/sql-commands/alter-subscription.md#sql-altersubscription-params-refresh-publication), e.g.:
