...
:   This record matches connection attempts made using TCP/IP. `host` records match SSL or non-SSL connection attempts as well as GSSAPI encrypted or non-GSSAPI encrypted connection attempts.
    ...
    !!! note
        ...
        Remote TCP/IP connections will not be possible unless the server is started with an appropriate value for the [listen_addresses](../server-configuration/connections-and-authentication.md#guc-listen-addresses) configuration parameter, since the default behavior is to listen for TCP/IP connections only on the local loopback address `localhost`.

`hostssl`
:   This record matches connection attempts made using TCP/IP, but only when the connection is made with SSL encryption.


     To make use of this option the server must be built with SSL support. Furthermore, SSL must be enabled by setting the [ssl](../server-configuration/connections-and-authentication.md#guc-ssl) configuration parameter (see [Secure TCP/IP Connections with SSL](../server-setup-and-operation/secure-tcp-ip-connections-with-ssl.md#ssl-tcp) for more information). Otherwise, the `hostssl` record is ignored except for logging a warning that it cannot match any connections.
...
*auth-method*
:   Specifies the authentication method to use when a connection matches this record. The possible choices are summarized here; details are in [Authentication Methods](authentication-methods.md#auth-methods). All the options are lower case and treated case sensitively, so even acronyms like `ldap` must be specified as lower case.

    `trust`
    :   Allow the connection unconditionally. This method allows anyone that can connect to the PostgreSQL database server to login as any PostgreSQL user they wish, without the need for a password or any other authentication. See [Trust Authentication](trust-authentication.md#auth-trust) for details.
    ...
    `scram-sha-256`
    :   Perform SCRAM-SHA-256 authentication to verify the user's password. See [Password Authentication](password-authentication.md#auth-password) for details.

    `md5`
    :   Perform SCRAM-SHA-256 or MD5 authentication to verify the user's password. See [Password Authentication](password-authentication.md#auth-password) for details.


        !!! warning

            Support for MD5-encrypted passwords is deprecated and will be removed in a future release of PostgreSQL. Refer to [Password Authentication](password-authentication.md#auth-password) for details about migrating to another password type.

    `password`
    :   Require the client to supply an unencrypted password for authentication. Since the password is sent in clear text over the network, this should not be used on untrusted networks. See [Password Authentication](password-authentication.md#auth-password) for details.

    `gss`
    :   Use GSSAPI to authenticate the user. This is only available for TCP/IP connections. See [GSSAPI Authentication](gssapi-authentication.md#gssapi-auth) for details. It can be used in conjunction with GSSAPI encryption.

    `sspi`
    :   Use SSPI to authenticate the user. This is only available on Windows. See [SSPI Authentication](sspi-authentication.md#sspi-auth) for details.

    `ident`
    :   Obtain the operating system user name of the client by contacting the ident server on the client and check if it matches the requested database user name. Ident authentication can only be used on TCP/IP connections. When specified for local connections, peer authentication will be used instead. See [Ident Authentication](ident-authentication.md#auth-ident) for details.

    `peer`
    :   Obtain the client's operating system user name from the operating system and check if it matches the requested database user name. This is only available for local connections. See [Peer Authentication](peer-authentication.md#auth-peer) for details.
    ...
    `pam`
    :   Authenticate using the Pluggable Authentication Modules (PAM) service provided by the operating system. See [PAM Authentication](pam-authentication.md#auth-pam) for details.

    `bsd`
    :   Authenticate using the BSD Authentication service provided by the operating system. See [BSD Authentication](bsd-authentication.md#auth-bsd) for details.

    `oauth`
    :   Authorize and optionally authenticate using a third-party OAuth 2.0 identity provider. See [OAuth Authorization/Authentication](oauth-authorization-authentication.md#auth-oauth) for details.
