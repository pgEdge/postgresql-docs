...
:   The [pgcrypto](../../appendixes/additional-supplied-modules-and-extensions/pgcrypto-cryptographic-functions.md#pgcrypto) module allows certain fields to be stored encrypted. This is useful if only some of the data is sensitive. The client supplies the decryption key and the data is decrypted on the server and then sent to the client.
