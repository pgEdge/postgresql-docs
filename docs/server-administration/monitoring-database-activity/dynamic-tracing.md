...
| `wal-buffer-write-dirty-start` | `()` | Probe that fires when a server process begins to write a dirty WAL buffer because no more WAL buffer space is available. (If this happens often, it implies that [wal_buffers](../server-configuration/write-ahead-log.md#guc-wal-buffers) is too small.) |
