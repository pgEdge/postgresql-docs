...
```
 ...
 For convenience, the programs [app-createuser](../../reference/postgresql-client-applications/createuser.md#app-createuser) and [app-dropuser](../../reference/postgresql-client-applications/dropuser.md#app-dropuser) are provided as wrappers around these SQL commands that can be called from the shell command line:
...
```
 The [app-psql](../../reference/postgresql-client-applications/psql.md#app-psql) program's `\du` meta-command is also useful for listing the existing roles.
