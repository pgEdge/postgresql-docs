...
| `GB18030` | National Standard, version 2022 | Chinese | No | No | 1–4 |  |
...
```
 Notice that the above commands specify copying the `template0` database. When copying any other database, the encoding and locale settings cannot be changed from those of the source database, because that might result in corrupt data. For more information see [Template Databases](../managing-databases/template-databases.md#manage-ag-templatedbs).
...
-  libpq ([Control Functions](../../client-interfaces/libpq-c-library/control-functions.md#libpq-control)) has functions to control the client encoding.
...
-  Using the configuration variable [client_encoding](../server-configuration/client-connection-defaults.md#guc-client-encoding). If the `client_encoding` variable is set, that client encoding is automatically selected when a connection to the server is made. (This can subsequently be overridden using any of the other methods mentioned above.)
