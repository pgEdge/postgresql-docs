...
-  You need a C compiler that supports at least C11. Recent versions of GCC are recommended, but PostgreSQL is known to build using a variety of compilers from different vendors.
...
-  The ICU library is used by default. If you don't want to use it then you must specify the `--without-icu` option to `configure`. Using this option disables support for ICU collation features (see [Collation Support](../localization/collation-support.md#collation)).
...
-  You need Curl to build an optional module which implements the [OAuth Device Authorization flow](../../client-interfaces/libpq-c-library/oauth-support.md#libpq-oauth) for client applications.
-  You need LZ4, if you want to support compression of data with that method; see [default_toast_compression](../server-configuration/client-connection-defaults.md#guc-default-toast-compression) and [wal_compression](../server-configuration/write-ahead-log.md#guc-wal-compression).
-  You need Zstandard, if you want to support compression of data with that method; see [wal_compression](../server-configuration/write-ahead-log.md#guc-wal-compression). The minimum required version is 1.4.0.
-  To build the PostgreSQL documentation, there is a separate set of requirements; see [Tool Sets](../../appendixes/documentation/tool-sets.md#docguide-toolsets).
