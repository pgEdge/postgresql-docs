...
:   Build with support for LLVM based JIT compilation (see [Just-in-Time Compilation (JIT)](../just-in-time-compilation-jit/index.md#jit)). This requires the LLVM library to be installed. The minimum required version of LLVM is currently 3.9. Disabled by default.
...
<a id="configure-spinlocks-meson"></a>
...
<a id="configure-atomics-meson"></a>
...
:   Enable test suites which require special software to run. This option accepts arguments via a whitespace-separated list. See [Additional Test Suites](../regression-tests/running-the-tests.md#regress-additional) for details.
