...
```
 Replace `/usr/local/pgsql/lib` with whatever you set ``--libdir`` to in [Configuration](building-and-installation-with-autoconf-and-make.md#configure). You should put these commands into a shell start-up file such as `/etc/profile` or `~/.bash_profile`. Some good information about the caveats associated with this method can be found at [http://xahlee.info/UnixResource_dir/_/ldpath.html](http://xahlee.info/UnixResource_dir/_/ldpath.html).
