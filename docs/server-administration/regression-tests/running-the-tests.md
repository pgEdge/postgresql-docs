...
```
 or otherwise a note about which tests failed. See [Test Evaluation](test-evaluation.md#regress-evaluation) below before assuming that a “failure” represents a serious problem.
...
-  Tests of client programs, located under `src/bin`.
 ...
 Some of these auxiliary test suites use the TAP infrastructure explained in [TAP Tests](tap-tests.md#regress-tap). The TAP-based tests are run only when PostgreSQL was configured with the option `--enable-tap-tests`. This is recommended for development, but can be omitted if there is no suitable Perl installation.
...
```
 ...
 This can be useful to enable additional logging, adjust resource limits, or enable extra run-time checks such as [debug_discard_caches](../server-configuration/developer-options.md#guc-debug-discard-caches).
