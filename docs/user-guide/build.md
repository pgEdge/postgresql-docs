...
**Download version v2.58.0 of  to /build path**
...
```bash
wget -q -O -
                    https://github.com/pgbackrest/pgbackrest/archive/release/v2.58.0.tar.gz |
                    tar zx -C /build
```
...
```bash
meson setup /build/pgbackrest /build/pgbackrest-release-v2.58.0
```
