...
```
 ...
 In a nonstrict function, if the actual value of an argument is null, the corresponding <code>$</code><em>n</em> variable will be set to an empty string. To detect whether a particular argument is null, use the function `argisnull`. For example, suppose that we wanted `tcl_max` with one null and one nonnull argument to return the nonnull argument, rather than null:
