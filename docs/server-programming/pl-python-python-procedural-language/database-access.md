...
<code>plpy.`execute`(</code><em>query</em><code> [, </code><em>limit</em><code>])</code>
...
<code>plpy.`prepare`(</code><em>query</em><code> [, </code><em>argtypes</em><code>])</code>, <code>plpy.`execute`(</code><em>plan</em><code> [, </code><em>arguments</em><code> [, </code><em>limit</em><code>]])</code>
:   `plpy.prepare` prepares the execution plan for a query. It is called with a query string and a list of parameter types, if you have parameter references in the query. For example:
    ...
    ```
     ...
     Query parameters and result row fields are converted between PostgreSQL and Python data types as described in [Data Values](data-values.md#plpython-data).


     When you prepare a plan using the PL/Python module it is automatically saved. Read the SPI documentation ([Server Programming Interface](../server-programming-interface/index.md#spi)) for a description of what this means. In order to make effective use of this across function calls one needs to use one of the persistent storage dictionaries `SD` or `GD` (see [Sharing Data](sharing-data.md#plpython-sharing)). For example:
...
<code>plpy.`cursor`(</code><em>query</em><code>)</code>, <code>plpy.`cursor`(</code><em>plan</em><code> [, </code><em>arguments</em><code>])</code>
