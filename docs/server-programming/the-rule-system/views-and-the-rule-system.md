...
### Updating a View
 ...
 Another possibility is for the user to define `INSTEAD` rules that specify substitute actions for `INSERT`, `UPDATE`, and `DELETE` commands on a view. These rules will rewrite the command, typically into a command that updates one or more tables, rather than views. That is the topic of [Rules on `INSERT`, `UPDATE`, and `DELETE`](rules-on-insert-update-and-delete.md#rules-update). Note that this will not work with `MERGE`, which currently does not support rules on the target relation other than `SELECT` rules.
