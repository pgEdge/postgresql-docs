...
`SPI_OK_REWRITTEN`
:   if the command was rewritten into another kind of command (e.g., `UPDATE` became an `INSERT`) by a [rule](../the-rule-system/index.md#rules).
...
## Description
 ...
 `SPI_execute_with_args` executes a command that might include references to externally supplied parameters. The command text refers to a parameter as <code>$</code><em>n</em>, and the call specifies data types and values for each such symbol. `read_only` and `count` have the same interpretation as in `SPI_execute`.
...
## Notes
 ...
 See the SQL [sql-fetch](../../reference/sql-commands/fetch.md#sql-fetch) command for details of the interpretation of the `direction` and `count` parameters.
