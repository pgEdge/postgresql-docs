...
`event`
:   Describes the event for which the function is called, one of `"login"`, `"ddl_command_start"`, `"ddl_command_end"`, `"sql_drop"`, `"table_rewrite"`. See [Overview of Event Trigger Behavior](overview-of-event-trigger-behavior.md#event-trigger-definition) for the meaning of these events.
