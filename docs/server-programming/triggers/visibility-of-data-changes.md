...
-  When a row-level `AFTER` trigger is fired, all data changes made by the outer command are already complete, and are visible to the invoked trigger function.
 ...
 Further information about data visibility rules can be found in [Visibility of Data Changes](../server-programming-interface/visibility-of-data-changes.md#spi-visibility). The example in [A Complete Trigger Example](a-complete-trigger-example.md#trigger-example) contains a demonstration of these rules.
