...
-  query language functions (functions written in SQL) ([Query Language (SQL) Functions](query-language-sql-functions.md#xfunc-sql))
-  procedural language functions (functions written in, for example, PL/pgSQL or PL/Tcl) ([Procedural Language Functions](procedural-language-functions.md#xfunc-pl))
-  internal functions ([Internal Functions](internal-functions.md#xfunc-internal))
-  C-language functions ([C-Language Functions](c-language-functions.md#xfunc-c))
 ...
 Throughout this chapter, it can be useful to look at the reference page of the [`CREATE FUNCTION`](../../reference/sql-commands/create-function.md#sql-createfunction) command to understand the examples better. Some examples from this chapter can be found in `funcs.sql` and `funcs.c` in the `src/tutorial` directory in the PostgreSQL source distribution.
