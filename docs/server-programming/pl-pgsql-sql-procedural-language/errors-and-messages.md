...
`ERRCODE`
:   Specifies the error code (SQLSTATE) to report, either by condition name, as shown in [PostgreSQL Error Codes](../../appendixes/postgresql-error-codes.md#errcodes-appendix), or directly as a five-character SQLSTATE code.
...
```
 ...
 Still another variant is to write `RAISE USING` or <code>RAISE
