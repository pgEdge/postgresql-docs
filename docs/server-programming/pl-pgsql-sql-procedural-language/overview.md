...
### Supported Argument and Result Data Types
 ...
 PL/pgSQL functions can be declared to accept a variable number of arguments by using the `VARIADIC` marker. This works exactly the same way as for SQL functions, as discussed in [SQL Functions with Variable Numbers of Arguments](../extending-sql/query-language-sql-functions.md#xfunc-sql-variadic-functions).


 PL/pgSQL functions can also be declared to accept and return the polymorphic types described in [Polymorphic Types](../extending-sql/the-postgresql-type-system.md#extend-types-polymorphic), thus allowing the actual data types handled by the function to vary from call to call. Examples appear in [Declaring Function Parameters](declarations.md#plpgsql-declaration-parameters).
 ...
 Specific examples appear in [Declaring Function Parameters](declarations.md#plpgsql-declaration-parameters) and [Returning from a Function](control-structures.md#plpgsql-statements-returning).
