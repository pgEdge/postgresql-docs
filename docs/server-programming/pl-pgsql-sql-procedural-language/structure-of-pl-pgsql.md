...
## Structure of PL/pgSQL
 ...
 Functions written in PL/pgSQL are defined to the server by executing [sql-createfunction](../../reference/sql-commands/create-function.md#sql-createfunction) commands. Such a command would normally look like, say,
...
!!! note
 ...
 It is important not to confuse the use of `BEGIN`/`END` for grouping statements in PL/pgSQL with the similarly-named SQL commands for transaction control. PL/pgSQL's `BEGIN`/`END` are only for grouping; they do not start or end a transaction. See [Transaction Management](transaction-management.md#plpgsql-transactions) for information on managing transactions in PL/pgSQL. Also, a block containing an `EXCEPTION` clause effectively forms a subtransaction that can be rolled back without affecting the outer transaction. For more about that see [Trapping Errors](control-structures.md#plpgsql-error-trapping).
