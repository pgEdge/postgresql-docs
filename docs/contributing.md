...
pgBackRest Contributing Guidelines.
...
!!! note
    ...
    `ERROR: [028]` regarding cache is invalid is OK; it just means there have been changes and the documentation will be built from scratch. In this case, be patient as the build could take 20 minutes or more depending on your system.
...
**List tests that don't require a container**
...
!!! note
    ...
    to build all the vms, just omit the `--vm` option above.
...
!!! note
    ...
    `HRN_*` macros should be used only for test setup and cleanup. `TEST_*` macros must be used for testing results.
...
!!! important
    ...
    A period (.) is required to end the `summary` section.
