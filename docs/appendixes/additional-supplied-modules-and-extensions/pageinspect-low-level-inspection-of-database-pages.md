...
:   `get_raw_page` reads the specified block of the named relation and returns a copy as a `bytea` value. This allows a single time-consistent copy of the block to be obtained. *fork* should be `'main'` for the main data fork, `'fsm'` for the [free space map](../../internals/database-physical-storage/free-space-map.md#storage-fsm), `'vm'` for the [visibility map](../../internals/database-physical-storage/visibility-map.md#storage-vm), or `'init'` for the initialization fork.
...
:   `bt_page_items` returns detailed information about all of the items on a B-tree index page. For example:
    ...
    ```
     ...
     For more details about the structure of B-tree indexes, see [B-Tree Structure](../../internals/built-in-index-access-methods/b-tree-indexes.md#btree-structure). For more details about deduplication and posting lists, see [Deduplication](../../internals/built-in-index-access-methods/b-tree-indexes.md#btree-deduplication).
