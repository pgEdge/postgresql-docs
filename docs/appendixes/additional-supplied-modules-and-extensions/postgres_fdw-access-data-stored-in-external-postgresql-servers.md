...
## postgres_fdw — access data stored in external PostgreSQL servers
 ...
 The functionality provided by this module overlaps substantially with the functionality of the older [dblink](dblink-connect-to-other-postgresql-databases.md#dblink) module. But `postgres_fdw` provides more transparent and standards-compliant syntax for accessing remote tables, and can give better performance in many cases.


 To prepare for remote access using `postgres_fdw`:

1.  Install the `postgres_fdw` extension using [sql-createextension](../../reference/sql-commands/create-extension.md#sql-createextension).
2.  Create a foreign server object, using [sql-createserver](../../reference/sql-commands/create-server.md#sql-createserver), to represent each remote database you want to connect to. Specify connection information, except `user` and `password`, as options of the server object.
3.  Create a user mapping, using [sql-createusermapping](../../reference/sql-commands/create-user-mapping.md#sql-createusermapping), for each database user you want to allow to access each foreign server. Specify the remote user name and password to use as `user` and `password` options of the user mapping.
4.  Create a foreign table, using [sql-createforeigntable](../../reference/sql-commands/create-foreign-table.md#sql-createforeigntable) or [sql-importforeignschema](../../reference/sql-commands/import-foreign-schema.md#sql-importforeignschema), for each remote table you want to access. The columns of the foreign table must match the referenced remote table. You can, however, use table and/or column names different from the remote table's, if you specify the correct remote names as options of the foreign table object.
...
-  `application_name` - this may appear in *either or both* a connection and [postgres_fdw.application_name](#guc-pgfdw-application-name). If both are present, `postgres_fdw.application_name` overrides the connection setting. Unlike libpq, `postgres_fdw` allows `application_name` to include “escape sequences”. See [postgres_fdw.application_name](#guc-pgfdw-application-name) for details.
...
-  [extra_float_digits](../../server-administration/server-configuration/client-connection-defaults.md#guc-extra-float-digits) is set to `3` for remote servers 9.0 and newer and is set to `2` for older versions
...
:   Specifies a value for [application_name](../../server-administration/server-configuration/error-reporting-and-logging.md#guc-application-name) configuration parameter used when `postgres_fdw` establishes a connection to a foreign server. This overrides `application_name` option of the server object. Note that change of this parameter doesn't affect any existing connections until they are re-established.
     ...
     `postgres_fdw.application_name` can be any string of any length and contain even non-ASCII characters. However when it's passed to and used as `application_name` in a foreign server, note that it will be truncated to less than `NAMEDATALEN` characters. Anything other than printable ASCII characters are replaced with [C-style hexadecimal escapes](../../the-sql-language/sql-syntax/lexical-structure.md#sql-syntax-strings-escape). See [application_name](../../server-administration/server-configuration/error-reporting-and-logging.md#guc-application-name) for details.
    ...
    | `%c` | Session ID on local server (see [log_line_prefix](../../server-administration/server-configuration/error-reporting-and-logging.md#guc-log-line-prefix) for details) |
    | `%C` | Cluster name on local server (see [cluster_name](../../server-administration/server-configuration/error-reporting-and-logging.md#guc-cluster-name) for details) |
...
```
 ...
 Then create a foreign server using [sql-createserver](../../reference/sql-commands/create-server.md#sql-createserver). In this example we wish to connect to a PostgreSQL server on host `192.83.123.89` listening on port `5432`. The database to which the connection is made is named `foreign_db` on the remote server:
...
```
 ...
 A user mapping, defined with [sql-createusermapping](../../reference/sql-commands/create-user-mapping.md#sql-createusermapping), is needed as well to identify the role that will be used on the remote server:
...
```
 ...
 Now it is possible to create a foreign table with [sql-createforeigntable](../../reference/sql-commands/create-foreign-table.md#sql-createforeigntable). In this example we wish to access the table named `some_schema.some_table` on the remote server. The local name for it will be `foreign_table`:
