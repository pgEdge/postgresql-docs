...
```
 ...
 For matching of single names, returned text arrays can be matched directly using the `&&` operator: any overlap can be considered a match. A GIN index may be used for efficiency, see [GIN Indexes](../../internals/built-in-index-access-methods/gin-indexes.md#gin) and this example:
...
```
 ...
 For indexing and matching of any number of names in any order, Full Text Search features can be used. See [Full Text Search](../../the-sql-language/full-text-search/index.md#textsearch) and this example:
