...
## Dependency Tracking
 ...
 To ensure the integrity of the entire database structure, PostgreSQL makes sure that you cannot drop objects that other objects still depend on. For example, attempting to drop the products table we considered in [Foreign Keys](constraints.md#ddl-constraints-fk), with the orders table depending on it, would result in an error message like this:
...
CREATE FUNCTION get_color_note (rainbow) RETURNS text AS
  ...
  LANGUAGE SQL;
```
 (See [Query Language (SQL) Functions](../../server-programming/extending-sql/query-language-sql-functions.md#xfunc-sql) for an explanation of SQL-language functions.) PostgreSQL will be aware that the `get_color_note` function depends on the `rainbow` type: dropping the type would force dropping the function, because its argument type would no longer be defined. But PostgreSQL will not consider `get_color_note` to depend on the `my_colors` table, and so will not drop the function if the table is dropped. While there are disadvantages to this approach, there are also benefits. The function is still valid in some sense if the table is missing, though executing it would cause an error; creating a new table of the same name would allow the function to work again.
