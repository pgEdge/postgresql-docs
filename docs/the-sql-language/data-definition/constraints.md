...
```
 ...
 A not-null constraint is functionally equivalent to creating a check constraint <code>CHECK (</code><em>column_name</em><code>
    IS NOT NULL)</code>, but in PostgreSQL creating an explicit not-null constraint is more efficient.
