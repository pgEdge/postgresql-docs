...
## Privileges
 ...
 The right to modify or destroy an object is inherent in being the object's owner, and cannot be granted or revoked in itself. (However, like all privileges, that right can be inherited by members of the owning role; see [Role Membership](../../server-administration/database-roles/role-membership.md#role-membership).)
...
```
 ...
 To assign privileges, the [sql-grant](../../reference/sql-commands/grant.md#sql-grant) command is used. For example, if `joe` is an existing role, and `accounts` is an existing table, the privilege to update the table can be granted with:
...
```
 ...
 The special “role” name `PUBLIC` can be used to grant a privilege to every role on the system. Also, “group” roles can be set up to help manage privileges when there are many users of a database — for details see [Database Roles](../../server-administration/database-roles/index.md#user-manag).


 To revoke a previously-granted privilege, use the fittingly named [sql-revoke](../../reference/sql-commands/revoke.md#sql-revoke) command:
...
```
 ...
 Ordinarily, only the object's owner (or a superuser) can grant or revoke privileges on an object. However, it is possible to grant a privilege “with grant option”, which gives the recipient the right to grant it in turn to others. If the grant option is subsequently revoked then all who received the privilege from that recipient (directly or through a chain of grants) will lose the privilege. For details see the [sql-grant](../../reference/sql-commands/grant.md#sql-grant) and [sql-revoke](../../reference/sql-commands/revoke.md#sql-revoke) reference pages.
...
`ALTER SYSTEM`
:   Allows a server configuration parameter to be configured to a new value using the [sql-altersystem](../../reference/sql-commands/alter-system.md#sql-altersystem) command.
