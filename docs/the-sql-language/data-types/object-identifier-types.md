...
| `regdatabase` | `pg_database` | database name | `template1` |
...
!!! note
    ...
    ```
     The `to_regclass()` function and its siblings can also be used to perform run-time lookups. See [System Catalog Information Functions](../functions-and-operators/system-information-functions-and-operators.md#functions-info-catalog-table).
...
```
 ...
 Another identifier type used by the system is `xid`, or transaction (abbreviated xact) identifier. This is the data type of the system columns `xmin` and `xmax`. Transaction identifiers are 32-bit quantities. In some contexts, a 64-bit variant `xid8` is used. Unlike `xid` values, `xid8` values increase strictly monotonically and cannot be reused in the lifetime of a database cluster. See [Transactions and Identifiers](../../internals/transaction-processing/transactions-and-identifiers.md#transaction-id) for more details.
 ...
 (The system columns are further explained in [System Columns](../data-definition/system-columns.md#ddl-system-columns).)
