...
```
 ...
 When a `timestamp with time zone` value is output, it is always converted from UTC to the current `timezone` zone, and displayed as local time in that zone. To see the time in another time zone, either change `timezone` or use the `AT TIME ZONE` construct (see [`AT TIME ZONE`](../functions-and-operators/date-time-functions-and-operators.md#functions-datetime-zoneconvert)).
