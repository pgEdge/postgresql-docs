...
:   ```
    ...
    ```
     ...
     <code>FROM </code><em>T1</em><code> CROSS JOIN
            </code><em>T2</em> is equivalent to <code>FROM </code><em>T1</em><code> INNER JOIN
            </code><em>T2</em><code> ON TRUE</code> (see below). It is also equivalent to <code>FROM </code><em>T1</em><code>,
            </code><em>T2</em>.

    !!! note

        This latter equivalence does not hold exactly when more than two tables appear, because `JOIN` binds more tightly than comma. For example <code>FROM </code><em>T1</em><code> CROSS JOIN
                 </code><em>T2</em><code> INNER JOIN </code><em>T3</em><code>
                 ON </code><em>condition</em> is not the same as <code>FROM </code><em>T1</em><code>,
                 </code><em>T2</em><code> INNER JOIN </code><em>T3</em><code>
                 ON </code><em>condition</em> because the *condition* can reference *T1* in the first case but not the second.
...
:   ```
    ...
    :   First, an inner join is performed. Then, for each row in T1 that does not satisfy the join condition with any row in T2, a joined row is added with null values in columns of T2. Also, for each row of T2 that does not satisfy the join condition with any row in T1, a joined row with null values in the columns of T1 is added.
     ...
     The `USING` clause is a shorthand that allows you to take advantage of the specific situation where both sides of the join use the same name for the joining column(s). It takes a comma-separated list of the shared column names and forms a join condition that includes an equality comparison for each one. For example, joining *T1* and *T2* with `USING (a, b)` produces the join condition <code>ON </code><em>T1</em><code>.a
            = </code><em>T2</em><code>.a AND </code><em>T1</em><code>.b
            = </code><em>T2</em><code>.b</code>.
...
FROM (VALUES ('anne', 'smith'), ('bob', 'jones'), ('joe', 'blow'))
     AS names(first, last)
```
 Again, a table alias is optional. Assigning alias names to the columns of the `VALUES` list is optional, but is good practice. For more information see [`VALUES` Lists](values-lists.md#queries-values).
...
```
 ...
 The special table function `UNNEST` may be called with any number of array parameters, and it returns a corresponding number of columns, as if `UNNEST` ([Array Functions and Operators](../functions-and-operators/array-functions-and-operators.md#functions-array)) had been called on each parameter separately and combined using the `ROWS FROM` construct.
...
```
 The [contrib-dblink-function](../../appendixes/additional-supplied-modules-and-extensions/dblink-connect-to-other-postgresql-databases.md#contrib-dblink-function) function (part of the [dblink](../../appendixes/additional-supplied-modules-and-extensions/dblink-connect-to-other-postgresql-databases.md#dblink) module) executes a remote query. It is declared to return `record` since it might be used for any kind of query. The actual column set must be specified in the calling query so that the parser knows, for example, what `*` should expand to.
...
```
 where *search_condition* is any value expression (see [Value Expressions](../sql-syntax/value-expressions.md#sql-expressions)) that returns a value of type `boolean`.
...
```
 ...
 The [`GROUP BY`](../../reference/sql-commands/select.md#sql-groupby) clause is used to group together those rows in a table that have the same values in all the columns listed. The order in which the columns are listed does not matter. The effect is to combine each set of rows having common values into one group row that represents all rows in the group. This is done to eliminate redundancy in the output and/or compute aggregates that apply to these groups. For instance:
...
(3 rows)
```
 Here `sum` is an aggregate function that computes a single value over the entire group. More information about the available aggregate functions can be found in [Aggregate Functions](../functions-and-operators/aggregate-functions.md#functions-aggregate).


!!! tip

    Grouping without aggregate expressions effectively calculates the set of distinct values in a column. This can also be achieved using the `DISTINCT` clause (see [`DISTINCT`](select-lists.md#queries-distinct)).
