...
### Extended Statistics
 ...
 Statistics objects are created using the [`CREATE STATISTICS`](../../reference/sql-commands/create-statistics.md#sql-createstatistics) command. Creation of such an object merely creates a catalog entry expressing interest in the statistics. Actual data collection is performed by `ANALYZE` (either a manual command, or background auto-analyze). The collected values can be examined in the [`pg_statistic_ext_data`](../../internals/system-catalogs/pg_statistic_ext_data.md#catalog-pg-statistic-ext-data) catalog.
