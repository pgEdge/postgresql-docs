...
```
 ...
 To force the planner to follow the join order laid out by explicit `JOIN`s, set the [join_collapse_limit](../../server-administration/server-configuration/query-planning.md#guc-join-collapse-limit) run-time parameter to 1. (Other possible values are discussed below.)
