...
<<   &<   &>   >>   <<|   &<|   |&>   |>>   @>   <@   ~=   &&
```
 (See [Geometric Functions and Operators](../functions-and-operators/geometric-functions-and-operators.md#functions-geometry) for the meaning of these operators.) The GiST operator classes included in the standard distribution are documented in [Built-in GiST Operator Classes](../../internals/gist-indexes/built-in-operator-classes.md#gist-builtin-opclasses-table). Many other GiST operator classes are available in the `contrib` collection or as separate projects. For more information see [GiST Indexes](../../internals/gist-indexes/index.md#gist).
...
```
 which finds the ten places closest to a given target point. The ability to do this is again dependent on the particular operator class being used. In [Built-in GiST Operator Classes](../../internals/gist-indexes/built-in-operator-classes.md#gist-builtin-opclasses-table), operators that can be used in this way are listed in the column “Ordering Operators”.
...
<<   >>   ~=   <@   <<|   |>>
```
 (See [Geometric Functions and Operators](../functions-and-operators/geometric-functions-and-operators.md#functions-geometry) for the meaning of these operators.) The SP-GiST operator classes included in the standard distribution are documented in [Built-in SP-GiST Operator Classes](../../internals/sp-gist-indexes/built-in-operator-classes.md#spgist-builtin-opclasses-table). For more information see [SP-GiST Indexes](../../internals/sp-gist-indexes/index.md#spgist).


 Like GiST, SP-GiST supports “nearest-neighbor” searches. For SP-GiST operator classes that support distance ordering, the corresponding operator is listed in the “Ordering Operators” column in [Built-in SP-GiST Operator Classes](../../internals/sp-gist-indexes/built-in-operator-classes.md#spgist-builtin-opclasses-table).
...
<@   @>   =   &&
```
 (See [Array Functions and Operators](../functions-and-operators/array-functions-and-operators.md#functions-array) for the meaning of these operators.) The GIN operator classes included in the standard distribution are documented in [Built-in GIN Operator Classes](../../internals/gin-indexes/built-in-operator-classes.md#gin-builtin-opclasses-table). Many other GIN operator classes are available in the `contrib` collection or as separate projects. For more information see [GIN Indexes](../../internals/gin-indexes/index.md#gin).
...
<   <=   =   >=   >
```
 The BRIN operator classes included in the standard distribution are documented in [Built-in BRIN Operator Classes](../../internals/brin-indexes/built-in-operator-classes.md#brin-builtin-opclasses-table). For more information see [BRIN Indexes](../../internals/brin-indexes/index.md#brin).
