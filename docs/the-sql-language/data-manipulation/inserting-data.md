...
## Inserting Data
 ...
 To create a new row, use the [sql-insert](../../reference/sql-commands/insert.md#sql-insert) command. The command requires the table name and column values. For example, consider the products table from [Data Definition](../data-definition/index.md#ddl):
...
```
 This provides the full power of the SQL query mechanism ([Queries](../queries/index.md#queries)) for computing the rows to be inserted.


!!! tip

    When inserting a lot of data at the same time, consider using the [sql-copy](../../reference/sql-commands/copy.md#sql-copy) command. It is not as flexible as the [sql-insert](../../reference/sql-commands/insert.md#sql-insert) command, but is more efficient. Refer to [Populating a Database](../performance-tips/populating-a-database.md#populate) for more information on improving bulk loading performance.
