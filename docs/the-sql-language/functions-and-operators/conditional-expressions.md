...
```
 ...
 The data types of all the *result* expressions must be convertible to a single output type. See [`UNION`, `CASE`, and Related Constructs](../type-conversion/union-case-and-related-constructs.md#typeconv-union-case) for more details.
...
```
 ...
 The arguments must all be convertible to a common data type, which will be the type of the result (see [`UNION`, `CASE`, and Related Constructs](../type-conversion/union-case-and-related-constructs.md#typeconv-union-case) for details).
...
```
 ...
 The two arguments must be of comparable types. To be specific, they are compared exactly as if you had written <em>value1</em><code>
   = </code><em>value2</em>, so there must be a suitable `=` operator available.
...
```
 ...
 The `GREATEST` and `LEAST` functions select the largest or smallest value from a list of any number of expressions. The expressions must all be convertible to a common data type, which will be the type of the result (see [`UNION`, `CASE`, and Related Constructs](../type-conversion/union-case-and-related-constructs.md#typeconv-union-case) for details).
