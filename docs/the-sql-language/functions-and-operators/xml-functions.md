...
```
 ...
 Element and attribute names that are not valid XML names are escaped by replacing the offending characters by the sequence <code>_x</code><em>HHHH</em><code>_</code>, where *HHHH* is the character's Unicode codepoint in hexadecimal notation. For example:
...
```
 ...
 To determine the order of the concatenation, an `ORDER BY` clause may be added to the aggregate call as described in [Aggregate Expressions](../sql-syntax/value-expressions.md#syntax-aggregates). For example:
...
```
 ...
 The `BY REF` and `BY VALUE` clauses are accepted in PostgreSQL, but are ignored, as discussed in [Incidental Limits of the Implementation](../../appendixes/sql-conformance/xml-limits-and-conformance-to-sql-xml.md#functions-xml-limits-postgresql).
 ...
 *document_expression* provides the context item for the *row_expression*. It must be a well-formed XML document; fragments/forests are not accepted. The `BY REF` and `BY VALUE` clauses are accepted but ignored, as discussed in [Incidental Limits of the Implementation](../../appendixes/sql-conformance/xml-limits-and-conformance-to-sql-xml.md#functions-xml-limits-postgresql).


 In the SQL standard, the `xmltable` function evaluates expressions in the XML Query language, but PostgreSQL allows only XPath 1.0 expressions, as discussed in [Queries Are Restricted to XPath 1.0](../../appendixes/sql-conformance/xml-limits-and-conformance-to-sql-xml.md#functions-xml-limits-xpath1).
...
!!! note
    ...
    XPath 1.0 does not specify an order for nodes in a node-set, so code that relies on a particular order of the results will be implementation-dependent. Details can be found in [Restriction of XPath to 1.0](../../appendixes/sql-conformance/xml-limits-and-conformance-to-sql-xml.md#xml-xpath-1-specifics).
 ...
 The conversion rules presented here are not exactly those of the SQL standard, as discussed in [Mappings between SQL and XML Data Types and Values](../../appendixes/sql-conformance/xml-limits-and-conformance-to-sql-xml.md#functions-xml-limits-casts).
...
```
 ...
 `table_to_xml` maps the content of the named table, passed as parameter `table`. The `regclass` type accepts strings identifying tables using the usual notation, including optional schema qualification and double quotes (see [Object Identifier Types](../data-types/object-identifier-types.md#datatype-oid) for details). `query_to_xml` executes the query whose text is passed as parameter `query` and maps the result set. `cursor_to_xml` fetches the indicated number of rows from the cursor specified by the parameter `cursor`. This variant is recommended if large tables have to be mapped, because the result value is built up in memory by each function.
