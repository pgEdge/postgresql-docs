...
<code>setweight(</code><em>vector</em><code> `tsvector`, </code><em>weight</em><code> `"char"`) returns `tsvector`</code>
...
<code>length(</code><em>vector</em><code> `tsvector`) returns `integer`</code>
:   Returns the number of lexemes stored in the vector.

<code>strip(</code><em>vector</em><code> `tsvector`) returns `tsvector`</code>
...
<code>tsquery_phrase(</code><em>query1</em><code> `tsquery`, </code><em>query2</em><code> `tsquery` [, </code><em>distance</em><code> `integer` ]) returns `tsquery`</code>
:   Returns a query that searches for a match to the first given query followed by a match to the second given query at a distance of exactly *distance* lexemes, using the <code><</code><em>N</em><code>></code> `tsquery` operator. For example:
...
<code>numnode(</code><em>query</em><code> `tsquery`) returns `integer`</code>
...
<code>querytree(</code><em>query</em><code> `tsquery`) returns `text`</code>
...
<code>ts_rewrite (</code><em>query</em><code> `tsquery`, </code><em>target</em><code> `tsquery`, </code><em>substitute</em><code> `tsquery`) returns `tsquery`</code>
...
<code>ts_rewrite (</code><em>query</em><code> `tsquery`, </code><em>select</em><code> `text`) returns `tsquery`</code>
...
!!! note
    ...
    The method described in this section has been obsoleted by the use of stored generated columns, as described in [Creating Indexes](tables-and-indexes.md#textsearch-tables-index).
