...
```
 ...
 The word `The` was recognized by the `english_ispell` dictionary as a stop word ([Stop Words](dictionaries.md#textsearch-stopwords)) and will not be indexed. The spaces are discarded too, since the configuration provides no dictionaries at all for them.
