...
The [Query Tool](query_tool.md#query_tool) and [View/Edit Data](#editgrid) tools are actually different operating modes of the same tool. Some controls will be disabled in either mode. Please see [The Query Tool Toolbar](query-tool/query_tool_toolbar.md#query_tool_toolbar) for a description of the available controls.
<a id="data-grid"></a>
...
*Tree Mode*: Enables changing, adding, moving, removing, and duplicating fields and values. Provides the ability to search and highlight data.

*Form Mode*: Allows editing only the values in JSON data, thereby providing the ability to keep the data structure unchanged while editing.

*Preview Mode*: Provides the ability to check data before saving and also shows the size of the current JSON data. Allows formatting and compacting JSON data as well.
...
| *Mode* | Click the *Mode* dropdown to change the display mode of the editor. Different modes available are Code, Preview, Tree, and Form. | Code, Tree, Form, Preview |
