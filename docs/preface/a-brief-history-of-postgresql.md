## A Brief History of PostgreSQL { #history }
 ...
 Another take on the history presented here can be found in Dr. Joe Hellerstein's paper “Looking Back at Postgres” [hell18](../bibliography.md#hell18).
...
-  The query language PostQUEL was replaced with SQL (implemented in the server). (Interface library [libpq](../client-interfaces/libpq-c-library/index.md#libpq) was named after PostQUEL.) Subqueries were not supported until PostgreSQL (see below), but they could be imitated in Postgres95 with user-defined SQL functions. Aggregate functions were re-implemented. Support for the `GROUP BY` query clause was also added.
