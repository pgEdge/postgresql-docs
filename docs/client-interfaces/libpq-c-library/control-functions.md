...
:   Sets the client encoding.
    ...
    ```
     *conn* is a connection to the server, and *encoding* is the encoding you want to use. If the function successfully sets the encoding, it returns 0, otherwise -1. The current encoding for this connection can be determined by using [PQclientEncoding](#libpq-PQclientEncoding).
<a id="libpq-PQsetErrorVerbosity"></a>

`PQsetErrorVerbosity`
:   Determines the verbosity of messages returned by [PQerrorMessage](connection-status-functions.md#libpq-PQerrorMessage) and [PQresultErrorMessage](command-execution-functions.md#libpq-PQresultErrorMessage).
...
:   Determines the handling of `CONTEXT` fields in messages returned by [PQerrorMessage](connection-status-functions.md#libpq-PQerrorMessage) and [PQresultErrorMessage](command-execution-functions.md#libpq-PQresultErrorMessage).
    ...
    ```
     ...
     Changing this mode does not affect the messages available from already-existing `PGresult` objects, only subsequently-created ones. (But see [PQresultVerboseErrorMessage](command-execution-functions.md#libpq-PQresultVerboseErrorMessage) if you want to print a previous error with a different display mode.)
...
`PQuntrace`
:   Disables tracing started by [PQtrace](#libpq-PQtrace).
