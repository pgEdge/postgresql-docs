...
-  [PQexec](command-execution-functions.md#libpq-PQexec) always collects the command's entire result, buffering it in a single `PGresult`. While this simplifies error-handling logic for the application, it can be impractical for results containing many rows.
 ...
 Applications that do not like these limitations can instead use the underlying functions that [PQexec](command-execution-functions.md#libpq-PQexec) is built from: [PQsendQuery](#libpq-PQsendQuery) and [PQgetResult](#libpq-PQgetResult). There are also [PQsendQueryParams](#libpq-PQsendQueryParams), [PQsendPrepare](#libpq-PQsendPrepare), [PQsendQueryPrepared](#libpq-PQsendQueryPrepared), [PQsendDescribePrepared](#libpq-PQsendDescribePrepared), and [PQsendDescribePortal](#libpq-PQsendDescribePortal), which can be used with [PQgetResult](#libpq-PQgetResult) to duplicate the functionality of [PQexecParams](command-execution-functions.md#libpq-PQexecParams), [PQprepare](command-execution-functions.md#libpq-PQprepare), [PQexecPrepared](command-execution-functions.md#libpq-PQexecPrepared), [PQdescribePrepared](command-execution-functions.md#libpq-PQdescribePrepared), and [PQdescribePortal](command-execution-functions.md#libpq-PQdescribePortal) respectively.
...
:   Waits for the next result from a prior [PQsendQuery](#libpq-PQsendQuery), [PQsendQueryParams](#libpq-PQsendQueryParams), [PQsendPrepare](#libpq-PQsendPrepare), [PQsendQueryPrepared](#libpq-PQsendQueryPrepared), [PQsendDescribePrepared](#libpq-PQsendDescribePrepared), [PQsendDescribePortal](#libpq-PQsendDescribePortal), or [PQpipelineSync](pipeline-mode.md#libpq-PQpipelineSync) call, and returns it. A null pointer is returned when the command is complete and there will be no more results.
 ...
 Another frequently-desired feature that can be obtained with [PQsendQuery](#libpq-PQsendQuery) and [PQgetResult](#libpq-PQgetResult) is retrieving large query results a row at a time. This is discussed in [Retrieving Query Results Row-by-Row](retrieving-query-results-row-by-row.md#libpq-single-row-mode).
...
:   Returns 1 if a command is busy, that is, [PQgetResult](#libpq-PQgetResult) would block waiting for input. A 0 return indicates that [PQgetResult](#libpq-PQgetResult) can be called with assurance of not blocking.
 ...
 A client that uses [PQsendQuery](#libpq-PQsendQuery)/[PQgetResult](#libpq-PQgetResult) can also attempt to cancel a command that is still being processed by the server; see [Canceling Queries in Progress](canceling-queries-in-progress.md#libpq-cancel). But regardless of the return value of [PQcancel](canceling-queries-in-progress.md#libpq-PQcancel), the application must continue with the normal result-reading sequence using [PQgetResult](#libpq-PQgetResult). A successful cancellation will simply cause the command to terminate sooner than it would have otherwise.
