...
`PQpipelineSync`
:   Marks a synchronization point in a pipeline by sending a [sync message](../../internals/frontend-backend-protocol/message-flow.md#protocol-flow-ext-query) and flushing the send buffer. This serves as the delimiter of an implicit transaction and an error recovery point; see [Error Handling](#libpq-pipeline-errors).
    ...
    ```
     ...
     Returns 1 for success. Returns 0 if the connection is not in pipeline mode or sending a [sync message](../../internals/frontend-backend-protocol/message-flow.md#protocol-flow-ext-query) failed.
<a id="libpq-PQsendPipelineSync"></a>

`PQsendPipelineSync`
:   Marks a synchronization point in a pipeline by sending a [sync message](../../internals/frontend-backend-protocol/message-flow.md#protocol-flow-ext-query) without flushing the send buffer. This serves as the delimiter of an implicit transaction and an error recovery point; see [Error Handling](#libpq-pipeline-errors).
    ...
    ```
     ...
     Returns 1 for success. Returns 0 if the connection is not in pipeline mode or sending a [sync message](../../internals/frontend-backend-protocol/message-flow.md#protocol-flow-ext-query) failed. Note that the message is not itself flushed to the server automatically; use `PQflush` if necessary.
