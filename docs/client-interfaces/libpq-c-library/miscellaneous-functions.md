...
:   Retrieves the number of bytes allocated for a `PGresult` object.
    ...
    ```
     ...
     This value is the sum of all `malloc` requests associated with the `PGresult` object, that is, all the space that will be freed by [PQclear](command-execution-functions.md#libpq-PQclear). This information can be useful for managing memory consumption.
