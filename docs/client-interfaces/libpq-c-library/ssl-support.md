...
:   Allows applications to select which security libraries to initialize.
    ...
    ```
     ...
     [PQinitSSL](#libpq-PQinitSSL) and [PQinitOpenSSL](#libpq-PQinitOpenSSL) are maintained for backwards compatibility, but are no longer required since PostgreSQL 18. [PQinitSSL](#libpq-PQinitSSL) has been present since PostgreSQL 8.0, while [PQinitOpenSSL](#libpq-PQinitOpenSSL) was added in PostgreSQL 8.4, so [PQinitSSL](#libpq-PQinitSSL) might be preferable for applications that need to work with older versions of libpq.
