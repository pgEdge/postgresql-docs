...
-  <em>username</em><code> USING </code><em>password</em>
 ...
 If the connection target includes any *options*, those consist of <em>keyword</em><code>=</code><em>value</em> specifications separated by ampersands (`&`). The allowed key words are the same ones recognized by libpq (see [Parameter Key Words](../libpq-c-library/database-connection-control-functions.md#libpq-paramkeywords)). Spaces are ignored before any *keyword* or *value*, though not within or after one. Note that there is no way to write `&` within a *value*.
