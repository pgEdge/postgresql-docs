...
**Table: Mapping Between PostgreSQL Data Types and C Variable Types**
...
| `decimal` | `decimal`<a id="ecpg-datatype-table-fn"></a>  (This type can only be accessed through special library functions; see [Accessing Special Data Types](#ecpg-special-types).) |
| `numeric` | `numeric`[^ecpg-datatype-table-fn] |
| `real` | `float` |
...
| `oid` | `unsigned int` |
| <code>character(</code><em>n</em><code>)</code>, <code>varchar(</code><em>n</em><code>)</code>, `text` | <code>char[</code><em>n</em><code>+1]</code>, <code>VARCHAR[</code><em>n</em><code>+1]</code> |
| `name` | `char[NAMEDATALEN]` |
| `timestamp` | `timestamp`[^ecpg-datatype-table-fn] |
| `interval` | `interval`[^ecpg-datatype-table-fn] |
| `date` | `date`[^ecpg-datatype-table-fn] |
| `boolean` | `bool` (declared in `ecpglib.h` if not native) |
| `bytea` | `char *`, <code>bytea[</code><em>n</em><code>]</code> |
 <a id="ecpg-char"></a>
...
```
 ECPG will report a syntax error for `START TRANSACTION`, because it no longer recognizes `START` as an SQL keyword, only as a typedef. (If you have such a conflict, and renaming the typedef seems impractical, you could write the SQL command using [dynamic SQL](dynamic-sql.md#ecpg-dynamic).)
...
##### Pointers
 ...
 You can declare pointers to the most common types. Note however that you cannot use pointers as target variables of queries without auto-allocation. See [Using Descriptor Areas](using-descriptor-areas.md#ecpg-descriptors) for more information on auto-allocation.
...
#### User-Defined Base Types
 ...
 Here is an example using the data type `complex` from the example in [User-Defined Types](../../server-programming/extending-sql/user-defined-types.md#xtypes). The external string representation of that type is `(%f,%f)`, which is defined in the functions `complex_in()` and `complex_out()` in [User-Defined Types](../../server-programming/extending-sql/user-defined-types.md#xtypes). The following example inserts the complex type values `(1,1)` and `(3,3)` into the columns `a` and `b`, and select them from the table after that.
...
```
 The indicator variable `val_ind` will be zero if the value was not null, and it will be negative if the value was null. (See [Oracle Compatibility Mode](oracle-compatibility-mode.md#ecpg-oracle-compat) to enable Oracle-specific behavior.)
