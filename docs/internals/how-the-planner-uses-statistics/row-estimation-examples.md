...
```
 How the planner determines the cardinality of `tenk1` is covered in [Statistics Used by the Planner](../../the-sql-language/performance-tips/statistics-used-by-the-planner.md#planner-stats), but is repeated here for completeness. The number of pages and rows is looked up in `pg_class`:
