...
ExecForeignTruncate(List *rels,
                    ...
                    bool restart_seqs);
```
 Truncate foreign tables. This function is called when [sql-truncate](../../reference/sql-commands/truncate.md#sql-truncate) is executed on a foreign table. `rels` is a list of `Relation` data structures of foreign tables to truncate.
...
```
 ...
 See [Row Locking in Foreign Data Wrappers](row-locking-in-foreign-data-wrappers.md#fdw-row-locking) for more information.
...
```
 This function is called when [sql-analyze](../../reference/sql-commands/analyze.md#sql-analyze) is executed on a foreign table. If the FDW can collect statistics for this foreign table, it should return `true`, and provide a pointer to a function that will collect sample rows from the table in `func`, plus the estimated size of the table in pages in `totalpages`. Otherwise, return `false`.
...
```
 Obtain a list of foreign table creation commands. This function is called when executing [sql-importforeignschema](../../reference/sql-commands/import-foreign-schema.md#sql-importforeignschema), and is passed the parse tree for that statement, as well as the OID of the foreign server to use. It should return a list of C strings, each of which must contain a [sql-createforeigntable](../../reference/sql-commands/create-foreign-table.md#sql-createforeigntable) command. These strings will be parsed and executed by the core server.
