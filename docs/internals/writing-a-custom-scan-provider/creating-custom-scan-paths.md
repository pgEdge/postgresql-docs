...
Plan *(*PlanCustomPath) (PlannerInfo *root,
                         ...
                         List *custom_plans);
```
 Convert a custom path to a finished plan. The return value will generally be a `CustomScan` object, which the callback must allocate and initialize. See [Creating Custom Scan Plans](creating-custom-scan-plans.md#custom-scan-plan) for more details.
