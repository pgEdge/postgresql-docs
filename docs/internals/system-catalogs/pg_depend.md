...
## `pg_depend`
 ...
 See also [`pg_shdepend`](pg_shdepend.md#catalog-pg-shdepend), which performs a similar function for dependencies involving objects that are shared across a database cluster.


**Table: `pg_depend` Columns**
...
<tr>
<td><p><code>refobjsubid</code> <code>int4</code></p>
<p>For a table column, this is the column number (the <code>refobjid</code> and <code>refclassid</code> refer to the table itself). For all other object types, this column is zero.</p></td>
</tr>
<tr>
...
:   The dependent object is a member of the *extension* that is the referenced object (see [`pg_extension`](pg_extension.md#catalog-pg-extension)). The dependent object can be dropped only via [`DROP EXTENSION`](../../reference/sql-commands/drop-extension.md#sql-dropextension) on the referenced object. Functionally this dependency type acts the same as an `INTERNAL` dependency, but it's kept separate for clarity and to simplify pg_dump.
