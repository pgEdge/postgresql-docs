...
# Generic WAL Records
 ...
 In comparison with [Custom WAL Resource Managers](custom-wal-resource-managers.md#custom-rmgr), Generic WAL is simpler for an extension to implement and does not require the extension library to be loaded in order to apply the records.


!!! note

    Generic WAL records are ignored during [Logical Decoding](../server-programming/logical-decoding/index.md#logicaldecoding). If logical decoding is required for your extension, consider a Custom WAL Resource Manager.
