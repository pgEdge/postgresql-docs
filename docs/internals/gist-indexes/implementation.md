...
### GiST Index Build Methods
 ...
 The sorted method is only available if each of the opclasses used by the index provides a `sortsupport` function, as described in [Extensibility](extensibility.md#gist-extensibility). If they do, this method is usually the best, so it is used by default.
