...
amcostestimate (PlannerInfo *root,
                ...
                double *indexPages);
```
 Estimate the costs of an index scan. This function is described fully in [Index Cost Estimation Functions](index-cost-estimation-functions.md#index-cost-estimation), below.
...
```
 ...
 The purpose of an index, of course, is to support scans for tuples matching an indexable `WHERE` condition, often called a *qualifier* or *scan key*. The semantics of index scanning are described more fully in [Index Scanning](index-scanning.md#index-scanning), below. An index access method can support “plain” index scans, “bitmap” index scans, or both. The scan-related functions that an index access method must or may provide are:
